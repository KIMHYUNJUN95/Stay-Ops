...
describe("getDisplayRoomLabel — 건물 접두어는 표시에서만 뗀다", () => {
  ...
  it("아라키초 _N 접미사를 먼저 떼고 접두어를 뗀다", () => {
    ...
    // `_N` 이 두 번 붙은 경우 (2026-09-16). Beds24 는 아라키초A 401호의 두 번째 유닛을
    // `401_2` 라고 부르는데 그 라벨을 **다카다노바바 556719** 가 이미 쓰고 있어
    // (라벨 유니크는 건물별이 아니라 조직 전체) 동기화가 `_2` 를 한 번 더 붙였다.
    // 저장값은 그대로 두고 여기서 뗀다 — 라벨을 바꾸면 청소 기록·예약 매칭이 끊긴다.
    expect(getDisplayRoomLabel("아라키초A", "401_2_2")).toBe("401");
  });
