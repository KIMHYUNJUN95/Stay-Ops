...
**모바일은 아직이다.** 현장 직원도 과거 날짜에서 "누가 했는지"를 봐야 하는데, 지금 RLS 는
`staff` / `part_time_staff` 에게 **본인 세션만** 보여준다(실측: 장지원이 볼 수 있는 세션 0/29).
그래서 모바일 확장은 **청소 세션 읽기 권한을 조직 구성원 전체로 넓히는 결정**이 선행되어야 한다.
같은 제약 때문에 지금도 모바일에서 남이 끝낸 방이 "대기"로 보여 **중복 청소 위험**이 있다.
