...
- **Shell height basis (2026-06-22 correction)**: the outer shell frame uses the live viewport height (`h-dvh`), while the centered wrapper and inner safe-area column inherit with `h-full` rather than each binding to their own viewport unit. The earlier all-`svh` approach caused real iPhone Safari bottom gaps under the tab bar and a sidebar floor that floated above the screen bottom. New shell-height work should keep one viewport-bound outer frame, not multiple nested viewport-bound boxes.
...
- **PWA install / icons / offline (2026-06-22)**: `public/manifest.webmanifest` has a full icon set (`/icons/icon-192`, `icon-512`, `maskable-512`), `id`/`scope` `/`, and `start_url: /mobile`; `layout.tsx` `metadata.icons` adds the iOS `apple-touch-icon`. Icons are generated from a brand SVG (navy squircle + ivory serif "S") by `scripts/dev/generate-pwa-icons.mjs` — replace with a real logo and re-run when one exists. A conservative service worker (`public/sw.js`, registered prod-only by `ServiceWorkerRegister` in `layout.tsx`) makes the app installable on Android and serves an `/offline` fallback; navigations stay **network-first** (no stale HTML/RSC), only content-hashed static assets are cached. Bump `CACHE` in `sw.js` to invalidate static cache on deploy.
- **Side menu**: tapping the menu button opens a left slide-out menu at roughly 78% of the mobile viewport width. The main screen moves right with a dark overlay (`bg-slate-950/42`) on the remaining visible area. Closing reverses the slide. **Scrim chrome-safe edge bands (2026-06-22):** the dismiss scrim is now mounted **only while the sidebar is open** and lives as a shell-local `absolute inset-0` layer inside the shared mobile shell (not a viewport-fixed layer). Its painted overlay leaves transparent top/bottom edge bands (`max(16px, env(safe-area-inset-*))`) and only dims the center span. Under `viewport-fit: cover`, and especially in regular Safari browser mode where the browser chrome is outside the safe-area model, a full-bleed dark scrim let iOS Safari sample the dark edge pixels and tint the status bar + URL toolbar black. Confining the scrim to the shell and unmounting it on close avoids the lingering top status-bar tint after the drawer closes, while the transparent edge bands still expose the ivory page background (`--background`) to Safari's chrome sampler in both browser mode and standalone. Layout top→bottom:
  - **Safari seam fixes (2026-06-22):** the sidebar no longer draws a bright `border-r` divider on its right edge. Depth separation comes from the panel shadow, because that 1px border could read as a white line against the dimmed scrim on Safari. Also, the scrim is now **display-mode aware**: in normal browser mode it uses `max(1px, env(safe-area-inset-*))` transparent edge rows so Safari samples the ivory page edge and keeps its chrome light; in installed standalone/home-screen mode it dims only the middle content region and leaves the shared status/header zone plus bottom-tab zone undimmed, so no horizontal seam or dark status-bar strip appears.
  - **Shared chrome while open:** when the side menu is open, the shared top bar and bottom tab bar temporarily slide/fade out instead of remaining dimly visible behind the drawer. In **standalone/home-screen mode**, the open drawer uses one continuous scrim over the exposed right-side area instead of preserving transparent header / tab-bar / safe-area bands. This removes the bright top-right and bottom-right horizontal blocks that made the drawer feel visually broken; the scrim unmounts on close, so there is no hidden layer left to affect the status bar afterward.
