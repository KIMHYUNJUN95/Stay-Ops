...
- **Side menu**: tapping the menu button opens a left slide-out menu at roughly 78% of the mobile viewport width. The main screen moves right with a dark overlay (`bg-slate-950/42`) on the remaining visible area. Closing reverses the slide. **Scrim chrome-safe edge bands (2026-06-22):** the dismiss scrim keeps a **full-screen click target** (`fixed inset-0`) but its painted overlay leaves transparent top/bottom edge bands (`max(16px, env(safe-area-inset-*))`) and only dims the center span. Under `viewport-fit: cover`, and especially in regular Safari browser mode where the browser chrome is outside the safe-area model, a full-bleed dark scrim let iOS Safari sample the dark edge pixels and tint the status bar + URL toolbar black. Leaving transparent edge bands exposes the ivory page background (`--background`) to Safari's chrome sampler in both browser mode and standalone while the visible center dim stays effectively unchanged. Layout top→bottom:
  - **Safari seam fixes (2026-06-22):** the sidebar no longer draws a bright `border-r` divider on its right edge. Depth separation comes from the panel shadow, because that 1px border could read as a white line against the dimmed scrim on Safari. Also, the chrome-safe transparent overlay rows use `max(1px, env(safe-area-inset-*))`, not a larger fallback band, so Safari still samples the ivory page edge without introducing visible horizontal bright seams near the top and bottom in browser mode.
