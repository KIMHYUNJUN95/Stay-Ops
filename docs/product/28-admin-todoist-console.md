...
| 뷰 | 내용 | 비고 |
| --- | --- | --- |
| 오늘 | 지연(일회성만) + 오늘 기준일 작업 + **반복(오늘 회차 또는 밀림)** | 반복(2026-07-30 회차 모델): 완료해도 롤포워드 안 함. **2026-09-08부터 반복은 지연 섹션에 뜨지 않는다** — 오늘 목록에 한 줄로 뜨고 밀림은 「N일 밀림」 배지. 완료하면 앞선 회차까지 흡수된다. 지연 섹션은 일회성 전용 |
| 내일 | 내일 기준일 작업 | |
...
**텍스트 입력은 비제어(uncontrolled)다 — 성능 계약이다.** 제목·설명·태그 버퍼는 `AddDraft` 에 없고
`addTextRef` 가 들고 있다. 제어 입력으로 두면 한 글자마다 `setAdd` 가 돌고, `add` 는 이 컴포넌트
(4900줄)의 state 라 **매 타자에 콘솔 전체가 다시 그려진다** — 작업 목록·캘린더·사이드바·팝오버까지.
연속 추가로 폼이 계속 열려 있게 되면서 그 비용이 「타자가 밀린다」로 드러났다(2026-09-04 제보).

화면이 실제로 알아야 하는 것은 **「제목이 비었는가」 하나**(저장 버튼 활성화)뿐이라 그것만 상태로
두고, 비었다↔찼다가 뒤집힐 때만 갱신한다. 저장·취소 시에는 `resetAddText()` 로 참조와 DOM 값을
함께 비운다.

**저장은 기다리지 않는다 (2026-09-04).** Enter 를 누르면 입력이 **즉시** 비워지고 새 작업이
**낙관적으로 목록에 얹힌다.** 서버 왕복(삽입 → `revalidatePath` → 페이지 데이터 7종 재조회 → RSC
재렌더)을 기다렸다가 비우면 다음 제목을 바로 칠 수 없다.

- 실패하면 `onError` 가 **본문을 되돌려 준다**(제목·설명·태그 버퍼·사진). 칩(시간·반복·우선순위·
  태그)까지 되살리지는 않는다 — 드문 경로이고 어느 시점 값인지 모호해진다.
- **이것이 이제 중복 제출 가드다.** 예전에는 `addSavingRef` 로 재진입을 막았는데(2026-08-25, 한 번
  탭에 두 건이 만들어지던 문제), 입력을 **동기적으로** 비우면 같은 틱의 두 번째 호출이 맨 위
  `!title.trim()` 에서 걸린다 — 더 강한 보장이면서 연속 추가를 막지 않는다. 저장 버튼도
  `pending` 으로 잠그지 않는다(잠그면 다음 건을 못 넣는다).
- 낙관적 행은 서버가 만들 값과 **같은 규칙**으로 조립한다(단일 날짜 모델 = `due_at`). 응답이 오면
  새 props 가 그 값을 대체하므로 실패해도 저절로 사라진다.

**React Compiler 도 이건 못 줄인다.** 상태가 바뀌면 그 상태를 읽는 트리는 다시 그려야 하고, 이
콘솔은 목록을 `Section({…})` / `renderRow(t)` 처럼 **함수 호출**로 그려 그 사이에 메모이제이션
경계가 생기지 않는다. 애초에 상태를 건드리지 않는 것이 유일한 해법이다. 새 입력을 이 폼에 추가할
때도 같은 규칙을 따를 것.
...
- **업무일지(보고서)는 본인 완료만** 집계(`generateDailyReport`)이므로, 보고서 모달 힌트 건수도 본인 기준으로 정정
  ...
  → **2026-09-09 일부 철회:** 「팀 완료」 중 **내가 보낸 지시**는 뺀다. 아래
  "완료·기록에서 보낸 지시 제외" 절 참고. 공유(공동 작업)의 팀 완료 표시는 그대로 유지된다.
...
## 콘솔 조망 범위 — 본인 참여 업무만 (2026-09-09 재확인 + 강제)

이 콘솔은 «큰 화면 Todoist»이지 팀 감사 도구가 아니다(§ Out of scope 의 "관리자 분석/오버사이트
레이어"). 따라서 콘솔이 보여 주는 것은 **뷰어 본인이 참여한 업무뿐**이다 — 본인 작업, 공유받은
작업, 받은 지시, 본인이 속한 프로젝트의 작업.

**2026-09-09 실제로는 새고 있었다.** 「오늘」 목록에 다른 사용자가 혼자 만든 개인 투두가
「작성자 → 제목」 형태로 섞여 있었다. 가시 범위 판정이 RLS 에만 맡겨져 있었고, `tasks` 의 select
정책은 `is_platform_admin()` 이면 참여자 조건을 건너뛰기 때문이다. 결과적으로 플랫폼 관리자
계정 하나가 조직 전원의 개인 투두를 오늘·내일·관리함·완료·기록에서 보고 있었다.

읽기 경로(`getVisibleTasks` / `getVisibleProjects` / `getTaskCompletions` / 회차 상태·순서 /
`getTaskDetail` / `getProjectDetail`)를 앱에서 참여 범위로 다시 좁혔다. 모바일과 콘솔이 **같은
함수를 공유**하므로 두 화면이 함께 닫힌다. 상세는
`docs/product/18-todo-task-workflow.md` → "2026-09-09 투두 가시 범위" 와
`docs/engineering/09-todo-task-technical-design.md` → "가시 범위 게이트".

> 팀 전체 조망이 필요해지면 그것은 **별도의 오버사이트 화면**으로 설계한다. 개인 투두 목록에
> 남의 할 일을 섞는 방식으로는 절대 만들지 않는다.

## 완료·기록에서 보낸 지시 제외 (2026-09-09)

**규칙.** 내가 **보낸** 지시를 대상자가 완료한 건은 **내 완료·기록에 뜨지 않는다.** 나는 지시자이고
작업자는 그 사람이다. 진행 상황은 「지시 › 보낸 지시」에서 본다.

- 내가 **직접** 완료한 건은 남긴다 — 그건 내가 한 일이다. 판정은
  `sentInstr(task, me) && completion.byUserId !== me` → 제외.
- **공유(공동 작업)의 팀 완료는 그대로 표시한다.** 공유는 둘 다 작업자이므로 2026-07-29 의
  "팀 완료를 기록으로 표시" 결정이 계속 유효하다. 이번 변경은 **지시에 한정**한 부분 철회다.
- 받은 지시는 내 일이므로 당연히 남는다.

**왜 여기만 새고 있었나.** 「보낸 지시는 대상자의 일정」이라는 규칙은 `myOwn()`
(`src/lib/task-directives.ts`)으로 이미 존재했고 오늘·내일·관리함·캘린더·레일에 적용돼 있었다.
완료·기록만 그 규칙을 통과시키지 않아서, 대상자가 완료하면 지시자의 기록에 남의 작업이 찍혔다.
