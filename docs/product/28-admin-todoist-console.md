...
**2026-08-03 Slack 업무일지 전송:** 완료·기록의 보고서 모달은 모바일과 같은 보고서 원문·권한을
공유하며, 편집한 본문을 단일 Slack 업무일지 채널로 수동 전송할 수 있다. 웹훅은 서버 환경변수만 사용하며
대시보드/모바일이 별도 복사본이나 별도 Slack 메시지 형식을 만들지 않는다. 보고서 푸터는 전송 라벨을
한 줄로 유지하며, 좁은 폭에서는 버튼 그룹을 다음 줄로 배치한다.

**2026-08-04 현장 활동:** 완료·기록 탭과 보고서에 **투두 밖에서 본인이 완료 처리한 일**(청소·유지보수·
린넨·주문)이 읽기 전용 줄로 함께 나온다. `tasks` 행은 만들지 않고 읽을 때만 합치며, 모바일과 같은
`src/lib/field-activity.ts` 를 쓴다. 검색·우선순위 필터가 걸리면 감춘다(작업 속성이 없어 필터가 안 먹는
것처럼 보이기 때문). 규칙 전체는 `docs/product/18-todo-task-workflow.md` → "Field Activity".

**2026-08-03 일지 항목 선택:** 보고서 모달 본문 위에 **포함할 항목** 체크리스트가 붙는다(기본 전체
체크). 체크를 풀면 그 항목이 본문에서 빠지고 **번호와 합계가 다시 매겨진다.** 헤더에 "8개 중 6개" 와
**전체 선택 / 전체 해제** 토글이 있고, 하나도 선택하지 않으면 복사·Slack 전송이 잠긴다. **"원본으로"는
본문과 선택을 함께 되돌린다** — 둘이 어긋나면 체크와 실제 전송 내용이 달라지기 때문이다. 조립은
모바일 `ReportSheet` 와 `src/lib/daily-report.ts` 를 공유하므로 두 화면의 번호·합계가 갈라지지 않는다.
상세는 `docs/product/18-todo-task-workflow.md` → "Daily Report (업무일지)".
...
### 2026-09-04 인라인 추가 — 저장해도 폼이 닫히지 않는다 (연속 추가)

한 건 넣고 곧바로 다음 건을 치는 것이 이 화면의 기본 흐름이다(Todoist 동일). 예전에는 Enter 나
「작업 추가」로 저장하면 폼이 **닫혀서**, 연속으로 넣으려면 매번 추가 버튼을 다시 눌러야 했다.

이제 저장 후에도 폼이 **그 자리에 남는다.** 새로 만든 작업은 목록에 추가되고 폼은 그 아래에
그대로 있으므로 바로 다음 제목을 칠 수 있다. 닫으려면 **「취소」 또는 Esc**.

**무엇이 남고 무엇이 비워지는가.**

| 유지 (어디에 넣을지) | 초기화 (무엇을 넣을지) |
| --- | --- |
| 탭·프로젝트·섹션(`ctx` / `sectionId` / `onDate`) | 제목 · 설명 · 태그 |
| 날짜(`date`) | 시간 · 기간 · 반복 · 우선순위 |
| 지시 탭의 대상자 | 사진 · 연결(건물/객실/예약) |

앞 작업의 **시간이나 반복이 다음 작업에 묻어가면 조용히 잘못된 일정이 생기므로** 그 둘은 반드시
비운다. 대상자는 **지시(`instr`) 탭에서만** 남긴다 — 그 탭은 대상이 없으면 저장 자체가 안 되고,
같은 사람에게 여러 건을 연달아 보내는 것이 본래 쓰임이다.

폼이 리마운트되지 않으므로 `autoFocus` 가 다시 걸리지 않는다. 제목 입력에 ref 를 두고 저장 후
직접 포커스를 돌려준다.
...
## 2026-08-03 「오늘로 이동」이 이미 오늘 걸린 반복에도 뜨던 문제

행 컨텍스트 메뉴의 오늘/내일 이동 항목을 **앵커 날짜(`dateOf`) 비교**로 정하고 있었다.
반복 작업의 앵커는 규칙의 시작일이라 오늘과 다를 수 있어(예: 평일 반복, 앵커 7/30, 오늘 8/3),
**이미 오늘 회차가 있는데도 「오늘로 이동」이 떴다.**

서버(`canMoveRecurringTo`)가 같은 날짜에 회차가 있으면 `duplicate_occurrence` 로 거절하므로
데이터가 망가지지는 않았지만, 눌러도 실패만 하는 메뉴였다.

판정을 **`occursOn(t, 날짜)`** 로 바꿨다 — 반복은 규칙으로, 일회성은 앵커 하루로 계산한다.
오늘·내일 양쪽에 회차가 있는 반복(평일 등)은 옮길 곳이 없으므로 항목을 아예 감춘다.

**모바일도 같이 정리했다.** 스와이프 목적지는 탭으로 정해지는데(오늘 탭 → 내일, 그 밖 → 오늘),
그 목적지에 이미 회차가 걸려 있으면 `TaskCard` 가 **스와이프 자체를 내주지 않는다**(`canSwipe`).
판정을 카드 안에 두어 모든 호출 지점(오늘·내일·지연·공유함·지시·프로젝트)이 한 번에 적용된다.
평일 반복은 오늘도 내일도 회차가 있어 스와이프가 사라지고, 주 1회 반복은 그대로 남는다.

참고: `task-card.tsx` 는 클라이언트 컴포넌트라 `@/lib/tasks` 를 **값으로 import 할 수 없다**
(그 모듈이 `supabase/server` → `next/headers` 를 끌어온다). 날짜 유틸은 파일 안에 사본으로 둔다.

## 검색 입력 지연 — 큰 트리 하나의 대가 (2026-08-11)

이 콘솔은 **4,500줄짜리 컴포넌트 함수 하나**이고 `useState` 44개에 `useMemo` 는 6개뿐이다.
검색어가 바뀌면 사실상 화면 전체가 다시 그려진다 — 한 글자 칠 때마다 전 작업 행·캘린더·
팝오버가 재렌더된다.

필터 계산에 **`useDeferredValue`** 를 끼웠다. 입력창은 즉시 반응하고, 무거운 목록만 한 박자
뒤에 낮은 우선순위로 따라온다. **로직은 하나도 바뀌지 않는다 — 스케줄링만 달라진다.**

- 입력값(`q`)은 입력창과 「필터 켜짐」 표시에 그대로 쓰고, 필터 계산만 `deferredQuery` 를 본다.
  둘을 섞으면 «지웠는데 칩이 안 꺼진다» 같은 어긋남이 생긴다.
- 모바일 `tasks-workspace.tsx` 도 같은 이유로 같은 처리를 했다.

### 컴포넌트를 물리적으로 쪼개는 것은 보류

«파일이 크다» 가 곧 «느리다» 는 아니었다. 실제 비용은 **재렌더 범위**였고 위 한 줄이 그걸 줄인다.
쪼개기는 유지보수 목적이며 회귀 위험이 있어 별도 작업으로 다룬다.

확인해 둔 것: 렌더 안에서 정의된 컴포넌트가 6개 있는데 **문제가 되지 않는다.**
입력을 가진 `InlineAdd` · `InlineAddSlot` · `Section` 은 `{InlineAdd()}` 처럼 **함수로 호출**돼
리마운트가 일어나지 않고, JSX 로 그려지는 `Avatar` · `Avatars` · `EmptyState` 는 상태가 없는
표시용이라 리마운트돼도 잃을 것이 없다. (입력을 가진 컴포넌트를 렌더 안에서 정의하고 JSX 로
그리면 매 렌더 언마운트→마운트가 일어나 **포커스가 튄다** — 여기서는 그 조합이 없다.)
