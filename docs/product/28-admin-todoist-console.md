...
**모바일에 같은 성격의 문제가 남아 있다.** 스와이프 이동 대상을 탭 기준
(`swipeActionForView`: 오늘 탭이면 내일로)으로 정하므로, 평일 반복 회차를 오늘 탭에서 스와이프하면
같은 이유로 서버가 거절한다. 스와이프는 어포던스라 "숨김/비활성" 중 무엇이 맞을지 별도 판단이 필요해
이번 범위에서 제외했다.
