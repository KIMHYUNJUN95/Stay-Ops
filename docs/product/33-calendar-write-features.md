...
> **아직 안 정한 것 — 쓸 때는 어느 유닛에 쓰는가.** 읽기는 위 규칙으로 메인을 몰라도 되지만,
> 가격을 **쓸 때는** 대상 유닛을 골라야 한다. 저쪽은 `PREFERRED_DUAL_ROOM_IDS` 로 두 곳만
> 코드에 박아 두었다(가부키초 803호 → 648398, 아라키초A 501호 → 502229). 오쿠보C 는 거기
> 없어서 목록의 첫 번째를 쓴다. 쓰기를 켤 때 이 표를 데이터로 옮기면서 다시 확인한다.
