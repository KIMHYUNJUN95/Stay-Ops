...
> once-per-Tokyo-day home prompt ("Still working?" suppresses; "Already left work" routes to correction, **no
...
> reason + desired times + desired site + memo + photos (optional); requests **only suggest values and never
...
> rows, review/abnormal markers). **Self-only** (server-scoped to the authenticated user). The history
> screen is **new UI in the existing design language** (the handoff had no history frame). See
...
- **Implemented (2026-07-14):** the 수당(시급) management screen (`/admin/attendance/wages`) also has a
  real Excel + PDF export (`exportAttendanceWagesWorkbook` / `exportAttendanceWagesReport`), replacing a
  previously disabled placeholder button. Same `owner` / `attendance_payroll_admin` gate as the payroll
  export. Two sheets: per-staff current-rate summary, and full rate-change history. See
  `docs/product/24-attendance-workflow.md` → "수당(시급) 화면 Excel/PDF 내보내기 구현".
...
- **18:30 리마인더는 외부 스케줄러에서 호출한다.** GitHub Actions가 Tokyo 18:30에 기존
  `/api/attendance/reminders` 엔드포인트를 실행하므로 앱 진입 여부에 의존하지 않는다.
...
## 2026-08-10 근태·급여 무결성 보강 규칙

이번 보강은 일본 법정 가산임금·법정 휴게·파트타임 연차 정책을 변경하지 않는다. 현재 StayOps가
이미 지원하는 출퇴근·휴게·시급 계산·월 마감 안에서 데이터가 조용히 누락되거나 부분 저장되는
문제만 차단한다.

- 월 마감은 **종료된 Tokyo 월**만 허용한다. 현재 월과 미래 월은 확정할 수 없다.
- 시급제 근무일은 고용형태와 시급 이력이 모두 해당 날짜를 덮어야 한다. 시급 공백이 하나라도
  있으면 0원으로 계산하지 않고 마감을 차단한다.
- 근태·휴게·시급·고용형태·수당·정정·스냅샷 조회 중 하나라도 실패하면 급여 계산과 마감을
  중단한다. 조회 오류를 빈 목록으로 해석하지 않는다.
- `invalid` 세션은 검토 필요 집계에서 제외한다. 무효 처리로 해소한 기록이 다시 마감을 막지 않는다.
- 완료 세션은 출근·퇴근 시각을 모두 가져야 한다. 둘 중 하나가 없으면 마감 대상이 아니다.
- 열린 휴게가 남은 세션은 상태와 관계없이 마감을 막는다. 관리자 정리 시 휴게 기록과 감사 기록을
  함께 남긴다.
- 정정 승인, 수기 근무 변경, 월 마감, 시급/고용형태 이력 교체는 본 기록과 감사 기록을 하나의 DB
  트랜잭션으로 저장한다. 부분 성공을 정상 처리하지 않는다.
- 퇴사·비활성 직원도 해당 조직의 과거 근태가 있으면 마지막 근무월을 마감할 수 있다.
- 분 단위 급여는 세션마다 초를 버리지 않고 같은 날짜의 인정 초를 합산한 뒤 1분 단위로 변환한다.
- GPS 판정은 위치 반경뿐 아니라 측정 정확도도 확인한다. 허용 정확도를 넘는 측정값은 재측정을
  요구하며 성공 기록으로 저장하지 않는다.
- 근태 리마인더는 앱 진입과 무관하게 외부 스케줄러에서 18:30 Tokyo 기준으로 호출한다.
