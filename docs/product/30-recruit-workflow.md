...
- ~~**Slack 웹훅 URL 이 채용 사이트 클라이언트 번들에** 들어 있다(`VITE_SLACK_WEBHOOK_URL`).~~
  **해소됨 (2026-09-24)** — 그 URL 을 쓰던 곳이 실시간 상담 하나뿐이었고, 상담 폐지와 함께 사라졌다.
...
지원서 **상세 패널 헤더**의 「인쇄」(닫기 왼쪽, 공용 `.panel__tool`). 한 번에 한 건이다.

하단 액션 바에 두지 않은 이유: 그 바의 왼쪽(삭제)·오른쪽(다음 단계로 진행) 자리는 반복 처리 중
오조작을 막으려고 고정돼 있다. 상태를 바꾸지 않는 인쇄를 그 사이에 끼우면 고정이 깨진다.

### 연락처를 **넣는다** — 내보내기와 다른 판단

| | 전화·카카오·주소 |
| --- | --- |
| 화면 | 기본 마스킹, 「연락처 표시」로 열람 |
| Excel · PDF 내보내기 | **제외** |
| **인쇄** | **포함** |

내보내기가 연락처를 빼는 이유는 「한번 나간 파일은 회수할 수 없다」였다. 인쇄물은 손에서 손으로만
움직이고, 종이 지원서를 대체하는 물건이라 면접장에서 본인 확인·연락에 바로 쓰인다. 같은 값을 두고
매체에 따라 판단이 갈리는 것이므로 여기에 명시해 둔다 — 나중에 「내보내기와 왜 다르냐」가 반드시
다시 나온다.

### 담기는 것

기본 정보(이름·나이·성별·국적·연락처) · 지원 공고(공고·고용형태·지원 직무·지원 경로) ·
근무 조건(요일·횟수·기간·시작일·통근·유니폼) · 체류 자격 · 경험 · 지원 동기 · 검토 메모.
헤더에 접수일 · 출력일.

- **빈 칸을 지우지 않고 「—」로 남긴다.** 종이에서 칸이 사라지면 「입력 안 한 사람」과
  「물어보지 않은 항목」이 구분되지 않는다(화면 패널과 같은 규칙).
- **현재 검토 단계(접수/서류검토/면접)는 넣지 않는다 (2026-09-24).** 종이는 지원자 정보를 보는
  물건이고, 심사 단계는 콘솔에서 바뀌므로 인쇄된 순간 이미 낡은 값이다.
- **이력서 첨부는 인쇄에 포함되지 않는다.** 별도 파일이고 형식도 제각각(PDF·이미지·문서)이라
  한 장에 합칠 수 없다. 필요하면 첨부 블록의 「열기 / 내려받기」로 따로 출력한다.

### 구현

`application-print-sheet.tsx` 가 화면에서 숨어 있는(`display:none`) A4 시트 `#rcSheet` 를 렌더하고,
`recruit-console.css` 의 `@media print` 가 그것만 남긴다. `休暇届`(`leave-documents-view.tsx` +
`admin-console.css` 의 `#docSheet`)와 같은 방식이다.

주의 둘:

- **`admin-console.css` 의 print 블록이 전역으로 `body * { visibility:hidden }` 을 건다.**
  `.adm` 스코프가 아니므로 이 페이지에도 적용된다. `#rcSheet` 를 id 선택자로 다시 보이게 해야 한다.
- **시트를 `position:fixed` 로 페이지 원점에 꺼낸다.** 패널 안에 둔 채로 인쇄하면 패널의
  `overflow`·`transform` 에 갇혀 한 귀퉁이만 나온다(休暇届 에서 이미 겪었다).

라벨은 패널이 쓰는 `copy.*` 를 그대로 재사용한다 — 인쇄 전용 문구를 따로 만들면 화면과 종이의
용어가 갈라진다. 새로 추가한 키는 9개(`printAction` · `printTitle` · `printAppliedAt` ·
...
- **지원서 탭은 이미 죽어 있다.** 2026-09-11 Firestore 읽기 차단 이후 403 만 받는다(위 "닫힌 것").
  화면만 남고 기능이 없는 상태이며, 사용자에게는 「고장난 관리자 페이지」로 읽힌다.
- **그 화면의 인증은 인증이 아니다.** 비밀번호가 `AdminPage.tsx` 에 상수로 박혀 브라우저에서 문자열
  비교만 한다. 살아 있는 한 번들에 비밀번호가 남는다. 고쳐 쓸 가치보다 없애는 편이 낫다.
- **관리 지점이 둘이면 반드시 어긋난다.** 이 문서가 처음부터 지켜 온 원칙(§2 "원본을 두 곳에서 쓰면
  반드시 어긋난다")을 화면 층에도 적용하는 것이다.
...
1. **문의 3건을 내보냈고, 미답변은 없었다.** 2건은 문의 폼만 제출하고 대화가 없었고, 1건은 담당자
   회신 후 지원자가 확인까지 한 종결된 대화였다. 화면의 빨간 뱃지 「3」 은 미처리 건수가 아니라
   `status: open` 인 방 개수였다. 내보낸 파일은 이름·전화번호가 담겨 있어 **저장소 밖**에 두었다.
2. **지원자 연락 수단은 남아 있다.** 채팅 위젯만 사라지고 창구 자체가 없어진 것은 아니다 —
   FAQ 하단(「담당자에게 카카오톡으로 연락해 주세요」)과 개인정보처리방침 11조(회사명·소재지·
   카카오톡 문의)의 안내가 계속 담당한다.

### 실제로 한 일 (haru-job-web `24c9d99`)

| 삭제 | |
| --- | --- |
| `src/pages/AdminPage.tsx` | 관리자 포털 451줄 |
| `src/components/Chat/*` | ChatWindow · InquiryForm |
| `src/hooks/useChat.ts` | Firestore 채팅 훅 |
| `src/utils/slack.ts` | 채팅 전용이라 함께 죽었다 |
| `admin.html` (저장소 루트) | 구 정적 어드민 630줄. 배포되지는 않았으나 Firebase 설정이 박혀 있었다 |

그 밖에 `App.tsx` 의 `/admin` 라우트, 푸터의 `Admin` 링크, `LandingPage` 의 채팅 배선(상태·핸들러·
「채용 문의」 CTA 카드·우하단 플로팅 버튼)을 걷어냈다. 합계 **−1,406줄**.

**곁가지로 보안 구멍 하나가 닫혔다.** §9 「아직 열려 있는 것」에 적혀 있던 **Slack 웹훅 URL 의 번들
노출**(`VITE_SLACK_WEBHOOK_URL`)이 사라졌다 — 그 URL 을 쓰던 곳이 채팅 하나뿐이었다.

**배포 확인.** 호스팅 사이트가 둘(`stayari.web.app` · `haru-recruit.web.app`)이고 **같은 번들을
서빙하는 복제본**이라 둘 다 배포해야 한다. `firebase.json` 의 hosting 항목이 각각 `site` 를 명시하고
있어 `--only hosting` 한 번으로 둘 다 나간다(타깃 매핑 불필요). 운영 번들이 로컬 빌드와 같은
해시(`index-DqAKsU8G.js`)인 것으로 확인했다.
...
쓴다. 채팅 폐지와 함께 `chat_rooms` 규칙도 잠그는 것이 맞다.

## Firestore 규칙 — `chat_rooms` 닫기 (2026-09-24, 배포 대기)

화면을 없애도 컬렉션은 남는다. 상담 폐지 시점에 `chat_rooms` 는 **여전히 인증 없이 읽혔다**
(실측 200). 이름·전화번호가 들어 있으므로 닫는다. 규칙 파일을 채용 사이트 저장소에 두었다
(`haru-job-react/firestore.rules`, `firebase.json` 에 등록).

**규칙을 쓰기 전에 현재 동작을 실측했다** — 규칙 파일이 없던 프로젝트라, 배포하면 콘솔에 있던
기존 규칙을 통째로 대체하기 때문이다. 인증 없는 REST 읽기 결과:

| 컬렉션 | 실측(2026-09-24) | 새 규칙 |
| --- | --- | --- |
| `applications` | 403 | `create` 만 허용 (지원 폼) |
| `applicants` | 403 | 전면 차단 |
| `chat_rooms` | **200 ← 열려 있었다** | **전면 차단** (하위 문서 포함) |
...
랜딩 페이지의 `trackVisit` 이 `siteStats/visitors_YYYY-MM-DD` 에 +1 하려 하지만 **이미 403 이다.**
2026-09-11 잠금 때 함께 닫힌 것으로 보인다. `trackVisit` 은 실패를 try/catch 로 삼키므로 화면에는
조용히 0 으로 뜬다.

**이번 규칙에서 다시 열지 않았다.** 목적은 `chat_rooms` 를 닫는 것이고, 지금 막혀 있는 것을 덤으로
여는 것은 별개의 결정이다 — 열면 누구나 카운터를 임의의 수로 쓸 수 있다. 카운터를 되살릴지는
따로 판단한다.
