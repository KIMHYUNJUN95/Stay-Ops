...
외부 채용 사이트에 들어온 지원서를 StayOps 로 받아 **심사하고, 합격자를 그대로 입사시키는** 흐름.
...
1. **심사를 StayOps 에서 한다** — 지원 현황이 운영 콘솔 안에 있어야 한다.
2. **채용 이력이 남는다** — 합격자의 이력서·지원서가 직원 기록으로 이어진다.
...
1. ~~`job_applications` 테이블 + RLS + 수신 웹훅~~ — 2026-09-09 완료
2. `/admin/recruit` 콘솔 — 목록·상세·상태 관리. 기존 어드민 프리미티브(칩 필터·테이블·상태 배지)와
   `.panel` 상세 패널을 재사용한다. 새 date picker·새 export 를 만들지 않는다(CLAUDE.md §4a·§4b).
