...
- **Dashboard Home** 가 데스크톱 운영 콘솔로 구현됨.
  ...
  - **사이드바 기본은 아이콘 레일이고, 마우스를 올릴 때만 펼쳐진다**(2026-09-24).
    평소 `64px`, 올리면 `248px`. 메뉴를 자주 쓰는 사람은 핀 버튼으로 **펼쳐 고정**할 수 있고
    그 선택은 기기에 기억된다(`localStorage: stayops.adm.side`).
    **기본이 레일이므로** 스크립트가 못 돌거나 localStorage 가 막힌 브라우저에서도 기본 동작이
    나온다 — 고정해 둔 사람만 한 번 펼쳐지는 것을 놓친다.
    ...
    - 상태는 뿌리 속성 `data-adm-side="wide"` 하나이고(**있을 때만** 펼쳐 고정)
      **보이는 것은 전부 CSS 가 판단한다** —
      React 가 그 값을 모르므로 hydration 이 어긋날 여지가 없다. 값은 사이드바 마크업보다 먼저
      실행되는 인라인 스크립트가 적용해 **페이지 이동마다 레이아웃이 튀는 것**을 막는다.
