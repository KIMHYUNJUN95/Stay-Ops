...
- **Dashboard Home** 가 데스크톱 운영 콘솔로 구현됨.
  ...
  - **사이드바는 접을 수 있다**(2026-09-24). `248px` ↔ `64px` 아이콘 레일, 기기에 기억된다
    (`localStorage: stayops.adm.side`). 판매 캘린더는 객실 90개 × 30일이 가로로 깔려 **248px 가
    날짜 약 5일치**이고, 눌러앉아 가격을 조정하는 동안 메뉴를 쓸 일이 거의 없다. 반대로
    청소·채용처럼 들락날락하는 화면에서는 사이드바가 제값을 하므로 **없애지 않고 접는다.**
    접힌 상태에서 메뉴 이름은 마우스를 올리면 뜨고, 그룹 제목 대신 가는 선이 묶음을 표시한다.
