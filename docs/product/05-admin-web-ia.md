...
- **Dashboard Home** 가 데스크톱 운영 콘솔로 구현됨.
  ...
  - **사이드바 기본은 아이콘 레일이고, 마우스를 올릴 때만 펼쳐진다**(2026-09-24).
    ...
    **기본이 레일이므로** 스크립트가 못 돌거나 localStorage 가 막힌 브라우저에서도 기본 동작이
    ...
    - **애니메이션 대신 지연**(`transition: … 0s 150ms`). 폭이 스르륵 늘어나면 그 동안 매
      프레임 리플로우가 일어나고, 지연이 없으면 마우스가 지나가기만 해도 펼쳐진다.
      글자도 같은 지연을 써서 폭과 **같은 순간**에 나타난다(`display:none` 은 지연시킬 수
      없어 투명도로 다룬다).
    - 그룹 제목이 투명해지는 자리는 가는 선이 묶음을 표시한다.
