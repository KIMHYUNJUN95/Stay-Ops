...
- 기본 조회는 이번 달(Tokyo)·최신 등록순이며, 건물·날짜/기간 필터를 제공한다.
- 수정은 건물·품목/수량·메모·사진만 가능하며, 원래 등록 시각·등록자는 증빙값으로 보존한다. 삭제는 MVP
  hard delete이며 확인과 감사 기록이 필요하다.
- 대시보드 v1에서 새 반품 등록, 상태, 품목 마스터, 집계, Excel/PDF export는 범위 밖이다.
- 구현·시각 디자인은 아직 시작하지 않았다. 예정 라우트는 `/admin/linen-return`, 사이드바는 운영 그룹이다.
