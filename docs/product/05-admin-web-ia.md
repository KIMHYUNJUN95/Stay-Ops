...
- **Dashboard Home** 가 데스크톱 운영 콘솔로 구현됨.
  ...
  - **사이드바는 접을 수 있고, 접으면 마우스를 올릴 때만 펼쳐진다**(2026-09-24).
    `248px` ↔ `64px` 아이콘 레일, 기기에 기억된다(`localStorage: stayops.adm.side`).
    ...
    - 상태는 뿌리 속성 `data-adm-side="rail"` 하나이고 **보이는 것은 전부 CSS 가 판단한다** —
