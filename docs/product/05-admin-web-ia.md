...
- **다이내믹 아일랜드·홈 인디케이터는 화면 위가 아니라 프레임(베젤) 쪽에 있다.** 실제 기기는
  `safe-area-inset-top` 이 내용을 밀어내지만 **iframe 안에서는 그 값이 0** 이라 겹치면 앱 헤더와
  부딪힌다. 피하려면 모바일 셸에 여백을 주입해야 하는데 그 셸은 공용 계약이라(CLAUDE.md §3)
  미리보기 때문에 실제 레이아웃을 건드릴 수 없다. 조금 덜 사실적이지만 **실제 앱 코드는 한 줄도
  바뀌지 않는다**(2026-09-11 사용자 선택 — A안).
