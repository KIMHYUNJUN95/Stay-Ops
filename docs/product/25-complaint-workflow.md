# Guest Feedback Workflow (게스트 피드백 — 수동 컴플레인·외부 리뷰)

> **기능 이름은 `게스트 피드백`이다 (2026-08-11 개명).** ko `게스트 피드백` / ja `ゲストフィードバック`
> / en `Guest Feedback`. 모바일 내비게이션만 탭바 폭 때문에 축약형(`피드백` / `ゲストの声` /
> `Feedback`)을 쓴다. 이전 이름 `컴플레인`은 이 모듈의 세 뷰 중 하나만 가리켰다 — 여기에는 좋은
> 평을 포함한 외부 리뷰 전체와 문제 객실 집계가 함께 들어 있다.
>
> **바뀐 것은 표시명뿐이다.** 라우트(`/admin/complaints`, `/mobile/complaints`), 내비 id
> (`complaints`), 테이블(`customer_complaints`), i18n 네임스페이스(`dictionary.complaints`),
> 그리고 컴플레인 **엔티티**를 가리키는 문구(`수동 컴플레인`, `컴플레인 등록`, `컴플레인으로 전환`,
> `컴플레인 삭제`)는 그대로다. 개명 대상은 기능 이름 하나이며, 엔티티 이름이 아니다. 이렇게 하면
> 사용자의 북마크와 저장된 `profiles.bottom_nav_tabs` 값이 깨지지 않는다.

## Purpose

StayOps의 게스트 피드백 영역은 다음 두 종류의 운영 데이터를 한곳에서 본다.
...
### External review (`external_reviews`, implemented)
...
## External Review Fields (implemented, 2026-08-04 API 조사 반영)
...
- **건물명은 집계가 현지화해서 내려보낸다 (confirmed 2026-08-07).** `summarizeReviewsByPlace` 는
  `locale` 을 받아 각 건물에 `name`(정규화 이름)과 `label`(현지화 표시명)을 함께 넣는다. `properties.name`
  은 `Okubo_A (B棟)` / `Okubo_C (kr)` 같은 Beds24 원본 표기라 그대로 화면에 나가면 캘린더·청소와 같은
  건물이 다른 이름으로 보인다. 변환 경로는 캘린더와 **동일**하다:
  `getCanonicalPropertyName` → `localizePropertyName` + `dictionary.cleaning.buildingLabels`.
  화면(어드민 표 / 모바일 카드)은 이 `label` 을 그대로 쓰고 각자 다시 매핑하지 않는다 — 화면별로
  매핑하면 한쪽만 빠뜨렸을 때 두 화면의 건물 이름이 갈라진다(실제로 어드민이 그렇게 빠져 있었다).
  사전에 없는 건물은 정규화 이름을 그대로 쓴다. **객실 라벨은 변환하지 않는다** — 현장 식별자 그대로다.
...
- `/mobile/complaints`: 수동 컴플레인과 외부 리뷰를 구분해 탐색할 수 있는 통합 진입점
  - **건물명은 화면에서 현지화한다 (2026-08-07).** `external_reviews.property_name` 에는 Beds24
    ...
    - 문제 객실 카드의 건물명은 집계가 준 `label` 을 쓴다(위 "Aggregation rule"). 화면에서 다시
      매핑하지 않는다.
- `/mobile/complaints/new`: 기존 수동 등록. 플랫폼·제목·내용·건물/객실/예약·고객명·사진 입력
- `/mobile/complaints/[id]`: 수동 컴플레인 상세, 상태·댓글·이미지
- `/mobile/complaints/reviews/[id]` (implemented): 외부 리뷰 원문, 원점수, 위험도, 건물/객실/예약 문맥,
...
`/admin/complaints`는 구현된 사무실/CS 통합 검토 콘솔이다. 공용 어드민 테이블·필터·우측 상세 패널
...
- **건물명은 모바일과 같은 라벨을 쓴다 (2026-08-07).** 문제 객실 표는 집계가 준 `label`, 외부 리뷰
  목록 행과 우측 상세 패널의 `건물` 항목은 `getCanonicalPropertyName` → `localizePropertyName`
  (`buildingLabels` 는 `/admin/complaints` 페이지가 `dictionary.cleaning.buildingLabels` 로 넘긴다).
  콘솔에 Beds24 원본 표기(`Okubo_A (B棟)`)를 노출하지 않는다.
...
- **기간 칩**은 공용 범위 캘린더 시트(`requests/date-range-calendar.tsx`)를 연다. 새 캘린더를
  ...
  - **선택 구간 띠의 끝은 둥글게 막는다 (2026-08-07).** 띠가 끊기는 자리는 세 군데다 — 주 경계
    (토→일), 달 첫날이 주중이라 앞이 빈칸인 자리, 기간이 다음 달로 이어져 격자 끝에서 잘리는
    자리. 이웃 칸의 띠가 맞닿는지 보고 안 닿는 쪽만 `rounded-*-full` 로 막는다. «시작·끝만
    둥글게» 로 하면 주 경계에서 각진 모서리가 그대로 남는다.
...
당시 다음 작업으로 분류했던 S3 리뷰 상세와 S4 문제 객실은 아래 후속 절의 현재 구현으로 완료됐다.
S5~S7 수동 컴플레인 화면과 3단 예약 피커, 삭제 확인 시트 역시 현재 구현을 기준으로 운영한다.
...
**후속 (2026-08-07): 165건 중 162건을 실제로 붙였다 — 65.2% → 1.2%.**

위 «객실을 자동으로 붙일 수는 없지만» 이라는 전제가 **틀렸다.** 붙일 수 있었다. 원인을 원인별로
세어 보니 셋으로 갈렸고, 둘은 해결 가능했다(설계·구현은
`docs/engineering/01-beds24-integration.md` → 「객실 재연결」).

| 원인 | 건수 | 결과 |
|---|---|---|
| 예약이 우리 DB에 없음(체크인 2026-04-22 이전 = 백필 창 밖) | 128 | **127건 연결** (Beds24 조회 4크레딧) |
| 리뷰가 예약보다 먼저 도착 → 수집 시점엔 객실 미상 | 34 | **34건 연결** (크레딧 0) |
| 취소된 예약 (조회에서 빠져 있었다) | 1 | **1건 연결** |
| 한 예약이 여러 객실에 걸침 | 3 | **의도적으로 비워 둔다** |

남은 3건은 «못 찾은» 것이 아니라 **찾지 않기로 한** 것이다 — 한 예약이 5개 객실에 걸쳐 있으면
그중 하나를 고르는 순간 **없는 문제를 다른 방에 씌우게 된다.** 예약 링크만 채우고 객실은 비워 둔다.
각주 UI 는 그대로 유지한다: 0이 아닌 이상 «합이 안 맞는 이유»는 여전히 필요하고, 앞으로도 이런
예약은 계속 생긴다(같은 예약자가 방을 여러 개 잡는 것은 반복되는 패턴이다).

**객실 칸 문구를 원인별로 나눈다 (2026-08-07).**

| 조건 | 문구 |
|---|---|
| 객실 있음 | 객실 라벨 |
| 객실 없음 + **예약 링크 있음** | **「다중 객실 예약」** |
| 객실 없음 + 예약 링크 없음 | 「객실 정보 없음」 |

- **행 자체가 신호다.** 예약을 찾으면 객실도 같이 확정되고, 못 찾으면 예약 링크도 비어 있다.
  그래서 «예약은 있는데 객실이 없다»는 조합은 **다중 객실 예약일 때만** 나온다. 후보를 계산하거나
  Beds24 를 부를 필요가 없다.
- **왜 나누는가:** 「객실 정보 없음」은 «우리가 못 가져왔다»로 읽혀 담당자가 **없는 수집 버그를
  찾게** 만든다. 원인이 다르면 문구도 달라야 한다.
- 판정은 `reviewRoomFallbackLabel()`(`src/lib/external-review-rules.ts`) 한 곳에 있다. 표시 지점이
  네 군데(모바일 목록·상세, 어드민 목록·상세)라 각자 판단하게 두면 화면마다 달라진다.
- 실측(2026-08-07): 2,468건 중 「다중 객실 예약」 3, 「객실 정보 없음」 **0**.

객실을 사람이 지정하는 기능은 **만들지 않는다.** 리뷰는 «외부가 말한 것의 사본»이라 추정이
섞이면 안 되고, 방을 확정해야 할 상황이라면 컴플레인으로 전환한 뒤 그쪽에서 다루는 것이 맞다
(`updateComplaint` 가 이미 `roomId` 수정을 지원한다). 빈도도 1.2% 다.
...
### S7 수동 컴플레인 등록 — 플랫폼 선택 신규 (2026-08-07)

**기능 공백을 메웠다.** 예전에는 `plat` 이 **예약을 연결할 때만** 설정돼, 연결 없이 등록한 건은
전부 `기타` 로 저장됐다. 전화·현장 접수처럼 예약을 못 고르는 경우가 오히려 흔한데도 그랬다.
플랫폼별 집계가 있는 제품에서 이건 데이터가 조용히 뭉개지는 문제다.

- 목업 2l 의 **2×2 그리드**(Airbnb / Booking / 직접예약 / 기타)를 제목 위에 둔다.
- **`null` 이 «기타» 다.** `PlatformKey` 에는 `other` 가 없고 제출 시 `plat ?? "other"` 로 저장되는
  기존 규약을 그대로 쓴다 — 이것 때문에 새 타입을 만들지 않는다.
- 플랫폼을 바꾸면 평점을 초기화한다. Airbnb 5점과 Booking 10점은 척도가 달라 이전 점수가 의미를 잃는다.
- **예약을 연결하면 피커가 플랫폼을 덮어쓴다.** 예약이 더 신뢰할 수 있는 출처다.

3단 예약 피커(건물 → 객실 → 게스트, 목업 2m·2n)는 v1 에 이미 구현돼 있어 그대로 둔다.

### 로딩 상태 · 날짜 표기 (2026-08-07)

**로딩 스켈레톤 (`src/app/mobile/complaints/loading.tsx`, 목업 2c).**
이 화면은 필터·페이지·집계를 서버에서 끝내고 내려오므로 첫 페인트까지 왕복이 한 번 있다. 그동안
빈 화면을 보여 주면 «데이터가 없다» 와 구분되지 않는다 — 리뷰가 2,400건 있는데도 빈 화면이면
고장으로 읽힌다. 카드 형태를 미리 그려 «오는 중»임을 알린다. 서버 컴포넌트라 `loading.tsx` 로
붙이고 클라이언트 상태를 만들지 않는다. `prefers-reduced-motion` 을 존중해 셔머를 끈다.

**카드 날짜는 `MM.DD` 두 자리 고정.**
`Intl` 의 numeric 포맷은 로케일마다 `8. 6.` / `8/6` 로 달라지고 자릿수도 흔들려, 목록에서 세로로
늘어놓으면 줄이 맞지 않았다. 카드는 «언제쯤인지»만 알면 되는 자리라 ISO 문자열을 잘라
`08.02` 로 고정한다. **상세 화면은 그대로 `2026.08.02 14:20` 까지 보여 준다** — 처리 기록과
시간대를 맞춰 봐야 하는 자리라 요구가 다르다.


## 기간 필터는 도쿄 날짜 기준이다 (2026-09-11)

게스트 피드백의 기간 선택기가 고른 날짜는 **도쿄 달력 날짜**이고, 리뷰 조회도 그 기준으로 자른다
(`applyReviewFilter` → `tokyoDayStart` / `tokyoDayEndExclusive`).

예전에는 `` `${from}T00:00:00Z` `` 로 잘라 **UTC 자정** 기준이었다. 도쿄가 UTC+9 이므로 결과가
9시간 밀려서,

- 시작일 **오전 0~9시(도쿄)에 달린 리뷰가 빠지고**,
- 종료일 **다음 날 오전 0~9시(도쿄) 리뷰가 섞여 들어왔다.**

리뷰 2,617건 중 **17.8%** 가 UTC 날짜와 도쿄 날짜가 다르기 때문에 경계일마다 실제로 숫자가
틀렸다. 실측 예:

| 범위 | 고치기 전(UTC 컷) | 고친 뒤(도쿄 컷) |
| --- | --- | --- |
| 2026-09-01 ~ 09-11 | 19건 | **21건** |
| 2026-08-24 ~ 08-31 | 42건 | **43건** |

KPI(기간 내 리뷰 / 문제 리뷰), 문제 객실 집계, 리뷰 목록이 **전부 같은 필터 함수**를 지나므로
한 곳을 고치면 세 곳이 같이 맞는다.

끝 경계는 `23:59:59` 가 아니라 **다음 날 0시 미만**(`lt`)이다. 초 단위로 자르면 그 1초 안의
소수점이 빠진다.

같은 결함이 채용 지원서의 접수 기간 필터에도 있었고 함께 고쳤다
