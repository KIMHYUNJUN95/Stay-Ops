...
**후속 (2026-08-07): 165건 중 161건을 실제로 붙였다 — 65.2% → 1.6%.**
...
| Beds24 에도 예약이 없음(2025-10-26) | 1 | 복구 불가 |

남은 4건 중 3건은 «못 찾은» 것이 아니라 **찾지 않기로 한** 것이다 — 한 예약이 5개 객실에 걸쳐
있으면 그중 하나를 고르는 순간 **틀린 객실에 문제를 귀속**시키게 된다. 예약 링크만 채우고 객실은
비워 둔다. 각주 UI 는 그대로 유지한다: 0이 아닌 이상 «합이 안 맞는 이유»는 여전히 필요하고,
앞으로도 이런 예약은 계속 생긴다.
