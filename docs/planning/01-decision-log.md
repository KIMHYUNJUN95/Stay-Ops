...
## 2026-09-10 (3) 채용 열람을 개인 지정으로 — 권한 모델의 첫 실전 적용

**사용자 답변으로 확정된 것.**

1. **대표·전무는 지정 없이도 본다.** 지정 목록이 비면 아무도 못 보게 되고, 담당자를 지정할 사람도
   그 화면을 봐야 할 때가 있다(잠금 방지).
2. **개발자는 모든 권한을 갖는다.** 「당연히 전부 있어야 한다」는 지시를 불변식으로 못박았다 —
   모든 키가 `platformBypass: true` 이고, **테스트가 전 키에 대해 검증**한다. 차단으로도 개발자를
   잠글 수 없다. 확인 결과 김현준 계정은 `owner` 멤버십과 **별개로** 활성 플랫폼 관리자여서 세션
   역할이 이미 `developer_super_admin` 으로 잡히고 있었다.
3. **현재 사무직 1명은 실제 채용 담당**이라 역할 기반 접근에서 **개인 지정으로 옮겼다**
   (`job_application.read` + `.triage`, 무기한). 넣고 빼는 것은 사용자가 권한 카드에서 직접 한다.
4. **읽기/쓰기를 나눴다(실무 기준).** 담당자에게는 열람 + 심사가 한 세트다 — 훑고 분류하는 것이
   콘솔이 하는 일 전부다. **삭제는 기본으로 주지 않는다**: 하드 삭제이고 이력서 파일까지 지운다.
   되돌릴 수 없는 개인정보 파기라 담당자라고 자동으로 가질 이유가 없다. 필요하면 개인 부여로 연다.

**바뀐 것.** `office_admin` 자동 열람이 사라졌다. 앞으로 사무직이 늘어도 지원서가 자동으로 열리지
않는다. 페이지 게이트·서버 액션·RLS 정책·사이드바가 **모두 같은 권한 키**를 본다
(`202609100002` 에서 RLS 를 `has_capability` 로 교체).

**전환 후 실측:** 개발자 전부 가능 / 담당자 열람·심사만(삭제 불가) / 파트타임 전부 불가.

**함께 전환한 것 — 사용자 관리(`user.manage`).** 원래부터 「지정된 개인만」인 화면인데(기본은
개발자, 개발자가 개인에게 위임) 위임 플래그 보유자가 0명이라 실제로는 개발자만 열 수 있으면서
메뉴는 전원에게 보였다. 권한 키로 옮겨 사이드바와 페이지가 같은 판정을 보게 했다.

Status: Confirmed (2026-09-10). 마이그레이션 `202609100002` 원격 적용 완료.
문서: `docs/product/30-recruit-workflow.md` §7, `docs/engineering/14-permission-architecture.md` §10.

## 2026-09-10 (2) 권한 모델을 역할 배열에서 권한 키로 — 개인 부여·차단을 같은 모델에

**사용자 방향 지시.** 「앞으로도 모든 기능은 역할 기반으로 나눠서 쓰는 기능도 필요하지만, 개개인을
특정해서 권한을 세분화할 수 있도록도 설계해야 한다. 채용 기능만 중요한 게 아니라 **설계 자체를**
그렇게 해야 한다. 차단 모델도 넣고, 문서도 확실하게 쓰고, 사용자 기능에서 관리할 수 있어야 한다.」

**발단.** 채용 지원서 열람 권한을 확인하다가 나왔다. 요구는 「사무직이든 현장직이든 **지정된 소수**
몇 명만」인데, 현재 모델은 역할로만 준다(`owner`/전무/`office_admin`). 직군으로 묶는 방식으로는
표현할 수 없는 요구였다.

**진단 — 문제가 넷이다.**

1. 「누가 쓸 수 있는가」가 역할 배열로 **네 곳에 복사**된다: 페이지 게이트 · 서버 액션 · RLS 정책
   (SQL) · 사이드바. 실측 정책 145개 중 31개가 역할 배열을 직접 들고 있다. 이 저장소는 판정식
   복사로 이미 여러 번 사고를 냈다.
2. 개인 단위 장치가 **네 종류**로 갈라져 있다: `membership_permission_overrides` 표,
   `attendance_payroll_admin`, `leave_approver_role`, `can_generate_report` 컬럼 3개.
3. **차단(deny)이 없다.** 더하기만 된다. 「사무직인데 이 사람만 제외」를 표현할 수 없어 역할을
   강등하는 수밖에 없고, 그러면 다른 권한까지 사라진다.
4. 사이드바가 권한을 모른다. `allowedRoles` 필드는 선언만 되고 배선되지 않은 죽은 필드다.

**결정.** 권한의 단위를 **권한 키(capability)** 로 바꾼다. 기능 코드는 역할을 모르고
`can(session, "job_application.read")` 만 묻는다.

```txt
can = NOT denied AND (granted OR role ∈ cap.roles OR (platform_admin AND cap.platformBypass))
```

- **차단이 최우선이다.** 「빼라」가 「줘라」보다 강해야 사고가 안전한 쪽으로 난다.
- 키마다 정책을 갖는다: `roles`(빈 배열이면 **지정된 개인만**) · `individualGrant` ·
  `individualDeny` · `requiresExpiry` · `platformBypass`.
- **기존 「기한 필수, 무기한 없음」 결정(27번 문서)은 폐기가 아니라 키별 정책으로 일반화**된다.
  예외성 권한은 그대로 기한 필수, 상시 업무 지정만 무기한.
- **잠금 방지:** owner·전무·개발자에게는 차단을 걸 수 없고, 자기 자신에게 부여·차단 불가,
  권한 관리 권한 자체는 개인 지정 대상이 아니다.
- **드리프트 방지가 설계의 핵심.** 역할표는 `src/config/capabilities.ts` 한 곳에만 있고,
  마이그레이션이 그것으로부터 `capability_roles` 를 채우며, 일치 테스트가 갈라짐을 잡는다.
  RLS 는 역할 배열 대신 `has_capability(org, user, key)` 만 부른다.
- 클라이언트는 스스로 계산하지 않는다. 서버가 세션에 **유효 권한 집합**을 실어 보내고 사이드바는
  그것만 본다.

**지금 하는 이유.** 실측 결과 오버라이드 **0행**, 기존 플래그 사용자 **0명**이다. 이전할 데이터가
없어 키 이름·스키마·기한 정책을 자유롭게 정할 수 있다. 부여 이력이 쌓인 뒤에는 같은 작업이 몇 배가
된다.

**관리 위치.** 새 화면을 만들지 않고 `/admin/users/[id]` 의 「권한 예외」 카드를 「권한」 카드로
확장한다 — 역할 변경이 이미 그 화면에 있다. **부여·차단·최종 유효 권한을 함께** 보여준다. 겹치면
사람은 결과를 암산하지 못하므로 화면이 판정식을 대신 계산해야 한다. ko/ja/en 동시.

**이행.** 1) 설계 문서 2) 토대(레지스트리·리졸버·SQL·세션·테스트) 3) 관리 UI 4) 사이드바
5) 기능 전환. **2~4단계에서는 어떤 권한도 바뀌지 않는다.** 5단계만 실제 권한이 움직이고,
기능마다 현재 게이트를 읽고 하나씩 옮긴다 — 일괄 전환은 「필요한 사람이 못 쓰게 되는」 방향으로
실패한다.
...
## 2026-09-10 채용 당겨오기 비용 최적화 — 조건 조회로 Firestore 읽기 50배 절감

**사용자 질문.** 「5분마다 불러오면 비용이 크지 않아?」

**실측.** 호출 1회 1.43초 · 하루 288회 · 1회당 Firestore 읽기 50건 = **하루 약 15,000건**.
무료 한도(5만/일)의 **30%** 다. 돈은 안 나가지만, 지원은 하루 한두 건인데 5분마다 최신 50건을
통째로 다시 읽고 있었다 — 99% 이상이 「이미 갖고 있는 걸 또 읽어 또 덮어쓰는」 일이었다.
(이력서는 재다운로드하지 않는 것을 확인했다: `ingest.ts` 가 `resume_path` 가 있으면 건너뛴다.)

**결정.** 목록 조회를 **조건 조회**(`runQuery` + `createdAt > since`)로 바꾼다. 기준은
`recruit_sync_state.last_success_at` 이라 새 저장소가 필요 없다.

| | 전 | 후 |
| --- | --- | --- |
| Firestore 읽기 | 15,000/일 (30%) | **약 300/일 (0.6%)** |
| Supabase 쓰기 | 14,400/일 | 새 지원서 수만큼 |

- **기준 시각을 10분 앞으로 당겨 겹치게 둔다.** 기준은 우리 서버 시각이고 `createdAt` 은 Firestore
  서버가 찍는다 — 시계가 같지 않고, 동기화가 도는 중에 저장된 문서도 있다. 딱 자르면 그런 문서를
  영원히 놓친다. 겹치면 같은 문서를 한두 번 더 읽을 뿐이고 수신은 재전송에 안전하다.
- 조건 조회는 `createdAt` 없는 구 문서를 못 보므로 **하루 1회 전량 훑기를 유지**한다.
...
사이트 재배포가 필요하고(그 저장소는 어드민 비밀번호가 번들 상수인 상태), 과거 배포 실패 이력이
있다. 「지원 오면 바로 전화」 운영으로 바뀌면 그때 다시 본다.

Status: Confirmed (2026-09-10). 실기 검증: 새 지원서 없을 때 `read: 0`, 기준을 9/01 로 되돌리면
7건(Firestore 직접 조회 결과와 일치). 문서: `docs/product/30-recruit-workflow.md` §2-1.
...
줄**로 보인다. 「반복은 지연되어도 안 뜨는 게 실무적으로 맞지 않나?」

**실측 화면이 근거였다.** 지연 9건 중 **8건이 반복**이고 전부 「1일 밀림」이었다. 평일 반복은 하루만
밀려도 전부 거기 쏟아지고, 주말·연휴를 지나면 더 심해진다. 그 결과 **진짜 조치가 필요한 것이
묻힌다** — 그 화면에서 실제로 급한 것은 8/24자 일회성 하나(2주 밀림)였는데 「1일 밀림」 8개 사이에
파묻혀 있었다. 이런 화면은 사람을 훈련시켜 지연 섹션 자체를 안 보게 만든다.

**결정(사용자 선택).** 반복은 **지연 섹션에 세우지 않는다.** 대신:

1. 반복이 **오늘 회차이거나 밀린 회차가 있으면** 오늘 목록에 **한 줄**로 뜬다
   (`showsOnTodayList`). 오늘이 회차가 아닌 반복(주간 등)이 밀린 경우도 오늘로 끌어온다.
2. 밀렸다는 사실은 그 줄의 **「N일 밀림」 배지**로 남는다. 정보가 사라지는 게 아니라, **행동 가능한
   항목에 붙는다.**
3. 오늘 것을 완료하면 서버가 **그보다 앞선 미해결 회차를 함께 해소**한다
   (`absorbEarlierOccurrences`) — 배지도 같이 사라진다.

**왜 「함께 해소」가 맞는가.** 이 반복 업무들은 누적되지 않는다 — 3일치 재고 확인을 세 번 하지
않는다. 2026-08-25 에 carry 중복을 고칠 때 이미 같은 판단을 했다. 오늘 한 번 하면 밀린 몫도 끝이다.

**상태는 `skipped` 가 아니라 `moved` 를 쓴다.** `moved_to_date` 에 「어느 날짜의 완료가 흡수했는가」
가 남아 나중에 되짚을 수 있다. `skipped` 는 사용자가 명시적으로 건너뛴 것을 뜻하므로 의미가 다르다.
**완료한 날짜보다 앞선 것만** 건드린다 — 캘린더에서 과거 회차를 직접 완료할 때 뒤의 미해결 회차까지
쓸어버리면 안 된다.

### 이 결정이 폐기한 것

「오늘로 가져오기」/「삭제」 2택(2026-07-30 결정 5항)이 **필요 없어졌다.** 완료가 곧 흡수이고,
안 할 거면 그냥 안 하면 배지가 남는다(그게 사실이다). 서버 액션 4개(`carryOverdueToToday` ·
`skipOverdueOccurrences` · `carryConsoleOverdueToToday` · `skipConsoleOverdue`)와 그 UI, 관련
i18n 키 4개(`odCarry` · `odSkip` · `odCarriedToOccurrence` · `odCarriedToNewTask`)를 함께 지웠다 —
호출부 0인 코드를 남기면 다음 사람이 «아직 쓰는 길이 있나» 를 다시 조사하게 된다. 순삭 131줄.

`createCarryOverTask` / `backlogCoveredByOccurrenceOn` 은 그 액션들에만 쓰이던 헬퍼라 함께 죽었고
import 정리로 드러났다.

### 같은 날 후속 — 「완료했는데 또 뜬다」 (사용자 제보)

배포 직후 제보: 오늘 반복을 **전부 완료했는데** 오늘 목록에 그 작업들이 「1일 밀림」 배지를 달고
미완료로 다시 떴다. 완료·기록 탭에는 6건 완료로 잡혀 있는데 오늘 목록에도 있으니 중복이다.

**원인은 `showsOnTodayList` 의 판정 순서였다.** 「오늘 회차가 열려 있는가」가 false 면 곧바로
「밀린 게 있는가」로 넘어갔다. 오늘 것을 **완료**해도 첫 조건은 false 이므로, 어제치 미해결 하나가
남아 있으면 그대로 목록에 섰다.

→ **오늘 회차가 이미 해결됐으면(완료·건너뜀·이동) 뜨지 않는다**를 맨 앞에 둔다. 밀린 게 남아
있어도 마찬가지다. 정상 흐름에서는 완료가 밀린 회차를 흡수하므로 배지도 곧 사라지지만, **이 판정은
흡수 여부와 무관하게 지켜져야 하는 불변식**이다 — 끝낸 일이 미완료로 다시 서면 안 된다.
회귀 가드로 테스트 6건을 붙였다(`showsOnTodayList` describe).

**기존 데이터 보정.** 이번 변경 전에 완료한 건들은 흡수가 돌지 않아 어제(9/07) 회차가 미해결로
남아 있었다 — 실측 7건, 전부 9/07 하루치. 새 코드가 자동으로 했을 일을 그대로 수행했다:
오늘 회차가 이미 해결된 반복에 한해 그 이전 미해결 회차를 `moved`(→ 2026-09-08)로 기록. 적용 후
잔여 0건 확인. 「일매출 변동 시트작업」은 월·수·금 규칙이라 오늘이 회차가 아니고, 밀린 것도 없다.

### 카운트·표시 동기화 (모바일 확인 요청에서 드러남)

「모바일도 같이 고쳤나」는 물음에 확인해 보니, 판정 자체는 **공유 모듈 `showsOnTodayList` 하나**를
두 화면이 부르므로 자동으로 함께 고쳐졌다. 그런데 **그 술어를 쓰지 않는 파생값**이 옛 규칙에 남아
있었다:

- **콘솔 상단 탭의 오늘 카운트** — 「오늘 회차」와 「지연 backlog」를 따로 세어 더하고 있었다.
  둘 다 가진 반복이 **2로 계상**된다(지금은 한 줄인데도). 목록과 같은 술어로 한 번만 세도록 고쳤다.
- **콘솔 탭의 「주의」 점** — 반복 밀림도 켜지는 조건이었다. 지연 섹션이 일회성 전용이 된 지금은
  일회성 지연이 있을 때만 켠다. 반복 밀림은 오늘 줄의 배지로 이미 보인다.
- 모바일 탭 카운트는 `todayRecBase` 를 그대로 쓰므로 이중 계상이 없었다(낡은 «두 줄» 주석만 정정).

**교훈.** 공유 술어로 모으면 판정은 한 번에 고쳐지지만, **그 술어를 우회해 직접 센 파생값**은 따로
남는다. 규칙을 바꿀 때는 «이 규칙을 다시 구현한 곳» 을 함께 훑어야 한다.

### 비회차 날짜 완료 — 「월수금 반복을 화요일에 보면?」 (사용자 확인 요청)

밀린 회차가 있으면 반복은 **오늘이 회차가 아니어도** 오늘 목록에 뜬다(월·수·금 반복을 화요일에
보는 경우). 그때 완료하면 어떻게 되는가 — 실측했더니 **규칙에 없는 화요일에 상태 행이 생기고
있었다**(일매출 변동 시트작업 / 2026-09-08 화, 사용자가 직전에 완료한 건).

목록은 맞게 보였다(회차 계산이 그 행을 무시하므로). 그러나 두 가지가 어긋난다:
1. 상세 화면은 `isRecurringOccurrenceDate` 로 회차를 검증하므로 그 날짜를 **무효로 보고 완료 버튼을
   감춘다** — 목록에서는 완료할 수 있는데 상세에서는 못 하는 상태.
2. **어느 회차를 했는지가 기록에 남지 않는다.** 실제로 한 일은 「밀린 월요일 몫」이다.

→ **`resolveCompletionOccurrence`**: 요청받은 날짜가 회차가 아니면 **가장 오래된 미해결 회차**로
정산한다. 화요일에 하는 일은 「밀린 월요일 것」이지 「화요일 것」이 아니다. 밀린 것도 없으면 기존
폴백을 그대로 둔다.

**되돌리기 대칭성도 함께 고쳤다.** 완료가 밀린 회차를 `moved_to_date = <완료일>` 로 흡수하는데,
되돌리기는 완료 행만 지웠다 — **취소해도 밀림 배지가 돌아오지 않았다.**
`releaseAbsorbedOccurrences` 로 그 완료일을 가리키는 `moved` 행을 함께 지운다. 이제 완료↔되돌리기가
대칭이다.

**데이터 정정.** 잘못 쓰인 화요일 행 1건을 실제 회차인 9/07(월)로 옮기고, 그때 흡수됐던 9/07 의
`moved` 행은 제거했다. 재검증 결과 규칙에 없는 날짜의 상태 행 **0건**. (앵커 이전 기록 23건이
남아 있는데, 이는 2026-08-25 에 고친 앵커 재작성 버그의 흔적이고 회차 계산이 앵커부터 시작하므로
무해하다.)

회귀 가드로 「월수금을 화요일에」 테스트 4건을 추가했다(208 → 212건).

### 손대지 않은 것

**일회성 지연은 그대로다.** 지연 섹션은 이제 일회성만 담고, 그래서 신호가 산다.

## 2026-09-08 (3) ④ RLS InitPlan 전면 적용 — 111개 정책, 권한 무변경 증명

`auth_rls_initplan` 을 DB 전체에 적용했다. 정책 안에서 `auth.uid()` 를 그냥 부르면 Postgres 가
**행마다** 재평가한다. `(select auth.uid())` 로 감싸면 InitPlan 이 되어 쿼리당 한 번이다.
인자가 없어 행에 무관한 `is_platform_admin()` 도 같다. 투두(2026-09-04)와 프로젝트·제안에서 먼저
적용했고 여기서 나머지 111개를 옮겼다.

### 손으로 쓰지 않은 이유, 그리고 어떻게 안전을 확보했나

정책을 다시 쓰는 일은 **「접근 권한을 잃는」 실패 모드**가 있다. 111개를 손으로 옮기면 오타 하나가
조용히 격리를 뚫거나 반대로 정상 접근을 막는다. 앞선 결정 로그에서 «한 번에 모든 정책을 다시 쓰는
것은 위험하다» 고 적어 둔 이유다.

그래서 **현재 정의 자체를 소스로** 삼는다. `pg_get_expr` 로 읽은 원문에서 인자 없는 두 함수만
텍스트 치환하고 `CREATE POLICY` 를 재조립한다 — 로직·역할·명령·permissive 가 원문 그대로 옮겨진다.
**행 값을 인자로 받는 헬퍼**(`has_active_membership(organization_id)`, `is_task_participant(id)` …)는
치환 대상이 아니므로 감싸지 않는다는 규칙이 자동으로 지켜진다(감싸면 상관 서브쿼리가 되어 손해다).

**증명.** 적용 전후로 「감싼 것을 되돌려 정규화한」 132개 정책 정의의 md5 를 비교했다:
`b456be6a85cbae7263e74de410105ae8` → **완전 일치.** 논리가 한 글자도 바뀌지 않았다는 뜻이다.
추가로 미감싸기 0건 · 이중 감싸기 0건, 활성 사용자별 가시 작업 수 분포도 그대로(88/0/0/0 —
83에서 늘어난 것은 그 사이 작업이 추가된 결과이고 분포는 동일).

### 마이그레이션을 DDL 나열이 아니라 DO 블록으로 둔 이유

111개 정책문을 파일에 박아 두면 **그 순간의 스냅샷**이 되어, 이후 다른 마이그레이션이 정책을 바꾸면
이 파일과 어긋난다. 자기 정의를 읽어 재작성하는 형태라 이미 감싼 것은 WHERE 절이 걸러내므로
**여러 번 실행해도 안전하다.**

이로써 전체 점검에서 나온 ①②③④ 가 모두 처리됐다.

## 2026-09-08 (2) 쓰기 실패가 조용히 묻히던 문제 — 위험도로 갈라 처리

전체 점검에서 **결과를 아예 받지 않는 쓰기 111곳**을 확인했다
(`await supabase.from(…).update(…)`, `const {error}` 없음). 실패하면 코드가 그대로 다음 줄로
넘어간다 — 사용자는 저장됐다고 믿고 데이터는 없다.

이 저장소는 이미 한 번 당했다. `setOccurrenceOrders` 에 남아 있는 주석: «초기 구현이 결과를 버려,
테이블 미적용 상태에서 저장이 조용히 실패했다. 화면은 낙관적으로 바뀌고 새로고침하면 되돌아가는데
아무 단서가 없었다.» 한 곳만 고치고 110곳이 남아 있었다.

### 111곳을 기계적으로 고치지 않은 이유

대부분은 감사 로그·알림·읽음 표시처럼 **실패해도 본 흐름을 막을 이유가 없는** 부수 기록이다
(`audit_logs` 9 · `attendance_session_audits` 6 · `board_post_reads` 2 · 웹훅 이벤트 3 …).
거기에 조기 반환을 심으면 오히려 정상 동작을 깨뜨린다. 111곳 일괄 수정은 churn 과 회귀 위험만 크다.

대신 **되돌릴 수 없는 후속 작업이 뒤따르는 쓰기**를 기계적으로 찾아(쓰기 다음 5줄 안에
`deleteUser` / `.delete()` / `redirect(`) 12곳을 추려 위험도로 갈라 처리했다.

### 신설: `src/lib/db-write-guard.ts`

- **`mustWrite(label, write)`** — 실패하면 `false`. 되돌릴 수 없는 후속 앞에서 쓴다. 호출부가
  중단·에러 리다이렉트를 고른다.
- **`bestEffortWrite(label, write)`** — 실패해도 진행하되 **조용하지는 않게**(경고 로그). 부수 기록용.

둘 중 하나를 고르는 것 자체가 «이 실패가 중요한가» 를 코드에 명시하는 일이다. 새 쓰기에 «그냥
await» 는 쓰지 않는다.

### Tier 1 — 되돌릴 수 없음 + 개인정보 (`deleteAccount`)

계정 삭제는 ① 프로필 묘비(PII 삭제) → ② 멤버십 제거 → ③ **auth 사용자 하드 삭제** 순인데, ①②가
결과를 받지 않았다. 실패해도 ③ 이 실행되므로 **개인정보(이름·전화·사진·생년월일)가 그대로 남고
조직 명단에도 계속 보이는데, 본인은 로그인조차 못 해 재시도할 수 없는** 상태가 된다.

→ ①② 를 `mustWrite` 로 감싸고 실패 시 `?error=delete_failed` 로 되돌린다. **되돌릴 수 없는 단계는
앞선 두 쓰기가 성공했을 때만 실행된다.** `onboarding.errors.delete_failed` 를 ko/ja/en 신설
(기존 폴백은 「프로필을 저장하지 못했습니다」로 상황에 맞지 않았다). 문구는 「변경된 것은 없으니
다시 시도해 주세요」 — 실제로 아무것도 바뀌지 않았음을 명시한다.

### Tier 2 — 고아 데이터 (섹션 삭제, 모바일·콘솔 2곳)

섹션을 지우면 하위 작업을 소프트 삭제한 뒤 섹션 행을 지운다. 앞이 실패하고 뒤가 성공하면 그
작업들은 **없는 섹션을 가리킨 채 남는다** — 어느 목록에도 안 뜨고 되돌릴 단서도 없다.
→ 하위 작업 정리를 `mustWrite` 로 확인한 뒤에만 섹션을 지운다.

### Tier 3 — 「성공한 것처럼」 돌려보내던 곳

- **작성자가 공유 작업에서 나가기** = 전체 소프트 삭제 → `?deleted=<id>` 로 보내 「삭제했습니다 ·
  실행 취소」 토스트를 띄운다. 실패했는데 그 토스트가 뜨면 **지워지지도 않은 작업을 되살리라고
  권하는 셈**이다. 성공했을 때만 그 경로로 보낸다.
- **오늘/내일로 이동**(스와이프) · **관리함 토글** · **섹션 추가/이름변경** — 실패해도 조용히
  목록으로 돌아가 「앱이 고장났다」로 읽혔다. 각 파일의 기존 에러 관례(`save_failed`)로 되돌린다.
- **`is_shared` 해제**(마지막 멤버·참여자가 빠질 때, 2곳) — 본 흐름을 막을 이유가 없어
  `bestEffortWrite`.

**곁가지로 찾은 문구 버그.** 이동 실패를 `moveError=save_failed` 로 돌려보내려 했더니, 워크스페이스의
이동 안내 토스트에 그 분기가 없어 **「그 날짜에는 이미 같은 반복 작업이 있습니다」** 라는 엉뚱한
...
**이 코드베이스는 오래전부터 React Compiler 가 켜져 있다고 전제하고 쓰여 왔다.**
`tasks-workspace.tsx` 에는 «React Compiler 가 메모이제이션을 보존하지 못하고 컴포넌트 전체를
포기한다»는 이유로 `useMemo` 를 넣거나 선언 순서를 맞춘 주석이 여럿 있다(116행 · 269행 · 371행).
그런데 실제로는 `next.config.ts` 에 설정이 없고 `babel-plugin-react-compiler` 도 설치돼 있지
않았다 — **꺼진 상태였다.** 즉 그 배려들이 아무 효과도 내지 못하고 있었다.

**결정: 손으로 쪼개는 대신 컴파일러를 켠다.**

두 대형 컴포넌트의 내부 함수들은 90개가 넘는 state 에 깊게 결합돼 있다. 파일로 분리하려면 props
인터페이스가 수십 개 필드로 불어나고, 앱을 실제로 띄워 확인할 수 없는 상태에서 그 규모의 수동
리팩터링은 회귀 위험이 매우 크다. 컴파일러는 **같은 문제를 자동으로, 훨씬 안전하게** 푼다.

- `next.config.ts` → `reactCompiler: true`, `babel-plugin-react-compiler` devDependency 추가.
- 검증: `react-compiler-healthcheck` 로 **앱 전체 536개 컴포넌트 중 536개 컴파일 성공**
  (4918줄 콘솔 · 2496줄 워크스페이스 포함). 호환 불가 라이브러리 없음. 프로덕션 빌드 통과.

**리마운트 문제는 원래 없었다.** 콘솔의 내부 컴포넌트들은 `<DetailPanel />` 이 아니라
`{DetailPanel()}` 처럼 **함수 호출**로 렌더된다(파일에 그 이유를 적은 주석도 있다). 새 컴포넌트
타입이 생기지 않으므로 언마운트/리마운트가 없다. 남은 비용이 «본문 전체 재실행»이었고, 그것이
바로 컴파일러가 없애는 비용이다.

**함께 정리:** `CTX_IC`(컨텍스트 칩 아이콘 상수)가 컴포넌트 안에 있어 렌더마다 재생성되고 있었다.
상태 의존이 없으므로 모듈 스코프로 옮겼다.

**남는 일.** 파일 크기 자체(4918줄)는 그대로다. 이제 성능 이유는 사라졌고 **가독성 이유만 남았으므로**,
앞으로도 큰 패널은 만질 때 하나씩 빼는 방식이 맞다 — 한 번에 쪼갤 이유가 없어졌다.

**주의:** 컴파일러 활성화는 빌드 파이프라인 변경이다. 되돌리려면 `next.config.ts` 의
`reactCompiler: true` 한 줄을 지우면 된다. 컴파일러는 올바른 React 코드에 대해 의미를 보존하지만,
**런타임 수동 QA 는 아직 하지 않았다.**

## 2026-09-03 (2) DB 타입 재생성 — 쓰기 타입 검사 복원, RLS 조직 스코프, 콘솔 조망 확정

구조 점검의 마지막 묶음(⑤ 근본 해결 · ⑦ · ⑧).

### ⑧ 콘솔 조망 범위 — **현행 유지 확정**

관리자 콘솔(`/admin/tasks`)이 조직 전체 업무를 볼지 물었고, **「지금대로 본인 참여 업무만」** 으로
확정했다(사용자 결정). 콘솔은 «데스크톱판 내 할 일»이지 조직 업무 조망이 아니다. 문서화된 기존
의도(`28-admin-todoist-console.md` → "모바일 7뷰를 그대로 데스크톱에")와 일치하므로 코드 변경 없음.
조직 전체 조망이 필요해지면 기존 7뷰를 바꾸지 말고 **관리 권한자 전용 뷰를 새로 추가**하는 쪽이
맞다 — 그래야 관리자 개인 할 일이 팀 업무에 묻히지 않는다.

### ⑦ RLS 조직 스코프 — 가시성 변화 없는 방어 강화

`tasks` 의 SELECT/UPDATE/DELETE 정책이 참여자 여부만 보고 `organization_id` 는 보지 않았다. 조직
격리가 애플리케이션 쿼리의 `.eq("organization_id", …)` 에만 의존하고 있었다는 뜻이다. 같은 테이블의
**INSERT 정책은 이미** `has_active_membership(organization_id)` 를 쓰고 있었으므로 설계 판단이 아니라
나머지 세 정책의 누락이다. 회차 테이블 둘(`task_occurrence_state`/`task_occurrence_order`)도 같았다.

`202609030002_task_rls_org_scope.sql` — 5개 정책 재작성. org 조건은 **일반 경로에만 AND** 로 붙여
플랫폼 관리자(`is_platform_admin()`)의 조직 간 조회는 그대로 뒀다.
적용 전 실측: 조직 활성 멤버가 아닌 참여자 0건(task 0 / project 0), `organization_id` 가 null 인 작업
0건. 적용 후 활성 사용자 4명의 가시 작업 수 **83 / 0 / 0 / 0 → 83 / 0 / 0 / 0** 로 완전 동일.

### ⑤ 근본 해결 — `src/types/database.ts` 재생성

2026-08-28 에는 `db-write.ts` 헬퍼로 «거짓말을 한 곳에 가두는» 우회를 했다. 그 원인이던 실제 결함
2건이 위 항목에서 해소됐으므로 이제 정본 타입을 도입했다.

재생성 후 에러는 **11건**, 전부 기계적이었다:
- **9건 — `GenericStringError`.** `const X = [...].join(", ")` 로 만든 select 문자열은 타입 수준에서
  파싱할 수 없어 결과가 에러 타입으로 떨어진다. `tasks.ts` 와 근태 4개 파일의 select 상수를
  **문자열 리터럴 + `as const`** 로 바꿨다. 캐스트를 붙이는 대신 고쳤으므로 이제 `.select()` 결과가
  **행 모양 그대로 타입이 잡힌다**(기존 `as TaskRow[]` 캐스트도 불필요해졌다).
- **2건 — 역할 배열.** `.in("role", …)` 에 `string[]` 을 넘기던 곳. `cleaning.ts` 는 «필터를 먼저
  돌리고 결과에 타입을 준다» 로 고쳤다 — 반대로 하면 `developer_super_admin` 을 걸러내는 그 필터가
  «항상 참» 이 되어 의미를 잃는다(TS 가 실제로 그렇게 지적했다).

그 다음 **`as never` 를 코드베이스 전체에서 걷어냈다(187곳)**. 남은 에러 15건도 전부 기계적이었고,
`Record<string, unknown>` 으로 느슨하게 들고 있던 쓰기 페이로드 9개를 **각 테이블의 정식
`Insert`/`Update` 타입**으로 바꾸는 과정에서 해소됐다. 상태 문자열 2건은 런타임 검증이 이미 있고
TS 가 좁히지 못한 것이라(`Set.has` 는 좁혀주지 않는다) 결과에만 타입을 부여했다.

**`db-write.ts` 는 삭제했다.** 그 파일의 존재 이유(«클라이언트 타입이 무너져 있다»)가 사라졌으므로
남겨두면 잘못된 설명이 붙은 불필요한 우회가 된다. 투두 4개 파일의 79개 호출부를 원래 형태로 되돌렸고,
`.insert({...})` / `.update({...})` 가 **캐스트 없이 네이티브로 타입 검사된다**(프로브로 확인).

실질 `as never` 잔존 0건(남은 5개는 주석 속 단어 또는 무관한 이중 캐스트). 66개 테이블 전체의 쓰기가
이제 컬럼명·타입 검사를 받는다.
...
성격이 「중복 생성」 / 「데이터 손실」 / 「반응성」 셋으로 갈린다.

1. **이동 액션이 반복 시리즈 앵커를 재작성해 지연 백로그를 소멸시켰다 (데이터 손실, 최우선).**
   `moveTaskToToday` / `moveTaskToTomorrow` / `moveConsoleToToday` / `moveConsoleToTomorrow` 가
   `recurrence_instance_date` 를 대상 날짜로 덮어썼다. 매주 월요일 반복을 수요일에 「오늘로 이동」
   하면 시리즈 전체가 매주 수요일이 되고, 밀린 회차는 앵커부터 계산되므로 **쌓여 있던 백로그가
   통째로 사라졌다.** 사용자에게는 아무 경고도 없었고 되돌릴 방법도 없었다.
   → **표준 반복 업무는 이 네 액션의 대상이 아니다.** 새 에러코드 `recurring_series` 로 거절하고,
   모바일 스와이프·콘솔 행 메뉴에서 항목 자체를 감춘다. 어떤 이동 경로도 더 이상
   `recurrence_instance_date` 를 쓰지 않는다(`anchorToDate` 및 콘솔 인라인 복제본 2곳 제거).
   앵커를 바꾸는 정당한 경로는 **사용자가 명시적으로 날짜를 고치는 편집**뿐이다. 이는 기능 축소가
   아니라 2026-07-30 「고정 앵커」 계약의 복원이다.

2. **「오늘로 가져오기」가 오늘 회차와 중복되는 사본을 만들었다.** 밀린 회차 범위는 `[앵커, 어제]`
   라 오늘 회차는 열린 채 남는데, 사본을 무조건 1건 만들어 매일·평일·사용자지정요일 규칙에서는
   오늘 목록에 같은 제목이 2건 떴다.
   → 오늘 회차가 **아직 열려 있으면** 그 회차가 보충분을 겸하므로 사본을 만들지 않는다. 오늘이
   회차가 아니거나 이미 완료/건너뜀/이동으로 해소됐으면 기존대로 만든다 — 그러지 않으면 밀린 일이
   조용히 증발한다. 판정은 공유 헬퍼 `hasOpenOccurrenceOn()`.
   덧붙여 사본이 `scheduled_date` 로만 앵커돼 있어, 그날 못 끝내면 다음 날 오늘·지연·내일 어디에도
   안 뜨고 관리함에만 남았다(지연 판정이 양쪽 화면 모두 `due_at` 만 본다). **단일 날짜 모델대로
   `due_at` 앵커로 교체**했다. 사본 생성은 `createCarryOverTask()` 로 통합 — 모바일·콘솔이 같은
   insert 블록을 복붙해 갖고 있던 쌍둥이였다.

3. **추가 버튼을 빠르게 두 번 누르면 작업이 2건 생성됐다.** 빠른 추가 시트·상세 작성 폼·콘솔
   인라인 추가 어디에도 전송 중 잠금이 없었고, 서버에도 멱등 키나 유니크 제약이 없었다. 상세 폼은
   `disabled={isPending}` 이 **사진 업로드 `await` 이후에야** 걸려 업로드 구간이 통째로 열려 있었다.
   → 세 경로 모두 **동기 ref 잠금**(state 는 리렌더가 늦어 첫 두 탭을 못 막는다) + 전송 중 버튼
   비활성화. 모든 조기 반환 경로에서 잠금을 해제해 실패한 제출이 폼을 잠그지 않게 했다.

4. **반복 업무를 상세에서 완료하면 엉뚱한 회차가 완료됐다.** 완료 버튼이 회차 날짜를 넘기지 않아
   서버가 시리즈 앵커로 폴백했고, 목록의 오늘 회차는 그대로 남았다. 버튼 라벨도 `task.status` 로
   판정했는데 반복 행의 status 는 영원히 `open` 이라 항상 「완료」로 보였다.
   → 회차 날짜를 전달하고, `done` 은 그 회차의 기록을 본다. 회차 컨텍스트 없이 열린 반복 상세
   (관리함의 시리즈 행)는 완료 버튼을 **숨기고** 오늘·내일·캘린더에서 열라고 안내한다 — 앵커로
   조용히 폴백하는 것이 바로 고치려던 버그다.

5. **콘솔 체크박스 완료가 느렸다.** 낙관적 갱신이 모바일에만 적용돼 있었고(2026-08-11), 콘솔은
   서버 왕복이 끝나야 반응했다. 게다가 `run()` 이 `revalidatePath` 로 이미 갱신된 화면을
   `router.refresh()` 로 한 번 더 돌려 **왕복이 2회**였다.
   → 모바일과 동일한 `useOptimistic` 모델을 이식하고, 콘솔의 모든 쓰기 액션이
   `revalidatePath(CONSOLE_PATH)` 를 호출함을 전수 확인한 뒤 중복 `router.refresh()` 를 제거했다.

**설계 판단 근거.** (1)에서 「거절」 대신 「회차만 이동」(대상 회차를 `moved` 로 기록 + 오늘자 사본
생성)도 검토했다. 부품은 이미 다 있으나 그것은 **새 기능**이라 데이터 손실 정지와 검증 범위를
섞지 않기로 했다. 또한 실제 피해가 발생한 관리함에는 회차 컨텍스트가 없어 그 기능을 붙여도 메뉴가
뜨지 않으므로, 실효 범위는 오늘·내일·캘린더로 한정된다. 후속 과제로 남긴다.

**후속(같은 날 처리).** 「오늘로 가져오기」가 사본을 만들지 않는 갈래에서는 화면에 새로 생기는
항목이 없어 «눌렀는데 아무 일도 없다»로 보였다. 두 액션이 결과(`{moved, carried}`)를 돌려주고
모바일·콘솔이 갈래별 문구를 띄우도록 했다(`odCarriedToOccurrence` / `odCarriedToNewTask`).

**기존 데이터 백필(2026-08-25 실행).** 이 수정 이전에 만들어진 carry 사본은 `scheduled_date` 로만
앵커돼 있어 화면에서 묻혀 있었다. 운영 DB 조회 결과 해당 행은 5건이고 그중 **열려 있는 1건**
(「주문요청 처리 (체크)」, 2026-08-20, 5일째 비노출)만 `due_at` 앵커로 옮겼다. 나머지 4건은 이미
완료 상태라 `due_at` 이 비어도 가리는 것이 없어 손대지 않았다. 되돌리려면 그 행의 `due_at` 을
null 로, `scheduled_date` 를 `2026-08-20` 으로 되돌리면 된다.

**남은 항목(미착수).** (a) 콘솔에서 관리자가 **남의** 반복 백로그를 당기면 사본의 작성자·참여자가
관리자 본인이 되어 담당 직원 목록에는 뜨지 않는다 — 사본을 누구 앞으로 만들지는 권한 정책 판단이라
분리했다. (b) 모바일 `today` 가 서버 렌더 시점 값이라 PWA 를 켠 채 JST 자정을 넘기면 stale 해진다.
...
- **이력서는 비공개 버킷**(`recruit-resumes`, `public = false`). 원본은 토큰이 박힌 Firebase
  다운로드 URL 이라 링크만 저장하면 URL 을 아는 누구나 열 수 있고, 채용 사이트를 접으면 링크가
  죽는다. 서버가 파일을 복사해 갖고, 열람은 서명 URL 로만 한다.
- **열람 권한은 owner / 전무 / office_admin.** 어드민 웹에 들어올 수 있는 전 역할에 열지 않는다.
- **재전송은 지원자 정보만 갱신한다.** 심사 상태·메모를 덮으면 「불합격 처리해 뒀는데 재전송 한 번에
  대기 중으로 돌아가는」 사고가 난다.
- **시크릿 미설정 시 수신 거부(503).** Beds24 웹훅은 미설정 배포를 견디려고 통과시키지만, 개인정보가
  들어오는 입구는 기본값이 거부여야 한다.
- **불합격자 자동 삭제 없음**(사용자 결정). 관리자가 직접 지운다.
...
데스크톱으로 들어와도 어드민 대시보드가 안 뜬다는 제보에서 출발했다. 원인은 라우팅 버그가 아니라
**조직에 존재하는 초대코드가 `part_time_staff` 하나뿐**이었던 것. `part_time_staff` 는
`adminWebRoles` 에서 유일하게 제외된 역할이라(2026-07-13 확정) 어떤 기기로도 `/admin` 이 열리지
않는다. 설계대로 동작한 셈이다.

문제는 **왜 그 코드 하나뿐이었나** 쪽이다. 초대코드 생성 폼이 조직·이름·코드·역할·만료일·최대
사용횟수 6개를 전부 손으로 채우게 되어 있어서, 다른 역할로 한 명 초대하려면 매번 코드 문자열을
새로 지어내야 했다. 그래서 처음 만든 파트타임 코드를 계속 재사용했다.

**결정: 폼에서 고르는 것은 조직과 역할 둘뿐. 이름·코드·만료일·최대 사용 횟수는 자동 생성한다.**

- 코드 `HARU-K7M2QP` — 조직 slug 앞 6자 + 랜덤 6자. `0 O 1 I L` 제외(구두·손글씨 혼동 방지)
- 이름 `사무직 초대 09-09` — 역할 라벨 + 도쿄 기준 생성일, ko/ja/en 각각
- 만료일 오늘 + 30일 / 최대 사용 횟수 10

**역할은 자동화하지 않는다.** 이번 사고의 원인이 정확히 「역할이 무엇인지 신경 쓰지 않고 만들어진
초대코드」였다. 역할은 초대코드가 결정하는 유일하게 되돌리기 번거로운 값이므로 매번 명시적으로
고르게 둔다.

**자동값은 덮어쓸 수 있게 남긴다.** 폼 아래 접힌 "세부 설정"에서 네 값 모두 수정 가능하고 코드는
↻ 로 다시 뽑는다. 완전히 제거하면 특정 코드 문자열이 필요한 경우(인쇄물에 이미 박힌 코드 등)를
막게 된다.

딸린 규칙 둘.

- **첫 코드는 서버가 뽑는다.** 클라이언트 첫 렌더에서 랜덤을 돌리면 하이드레이션이 깨진다.
- **자동 코드 충돌은 조용히 재시도(최대 5회), 사용자가 넣은 코드 충돌은 `duplicate_code` 로 알린다.**
  사용자가 지정한 코드를 말없이 바꿔치기하지 않는다.

구현: `src/lib/invite-code-gen.ts`(공유 규칙) / `invite-create-form.tsx`(폼) /
`createInviteCode`(빈 값 서버 측 보정). 자세한 내용은 `docs/product/05-admin-web-ia.md`.

## 2026-09-09 온보딩 위저드 — 데스크톱 입력 수단을 갖춘다

「PC 로 회원가입할 때 생년월일이 마우스 휠로 안 움직이고 클릭으로만 된다」는 제보. 확인해 보니 휠
컴포넌트가 iOS 손가락 스크롤만 상정하고 있었다 — 항목은 클릭되지 않는 `<div>`, 스크롤바는 숨김,
키보드 진입점 없음. 마우스 휠은 네이티브 스크롤에만 기댔는데 스냅 스크롤 + 110ms 후 `scrollTop`
보정과 관성 스크롤이 서로를 밀어냈다.

**결정: 온보딩 위저드는 모바일 우선이지만 데스크톱을 «되기는 한다» 수준으로 두지 않는다.**
PC 가입은 실제로 쓰이는 경로다.

- 휠은 네이티브 스크롤에 맡기지 않고 `wheel` 을 직접 받아 칸 단위로 움직인다. React 의 `onWheel`
  은 루트에 passive 로 붙어 `preventDefault` 가 먹지 않으므로 **네이티브 리스너**여야 한다.
- 클릭 선택 · 키보드(↑↓/PageUp·Down/Home·End/Enter) · 숫자 타이핑 점프를 함께 붙인다.
- 이름·전화·초대코드 입력에서 **Enter 로 다음 단계**. 단독 `<input>` 이라 폼 제출이 없어 PC 에서
  매번 마우스로 하단 버튼을 눌러야 했다.

**뒤로 버튼은 원래 결정을 뒤집는다.** 코드 주석에 "back chevron 없음 — 스와이프 / OS 백이 공통
패턴"으로 박혀 있었으나, PC 에는 엣지 스와이프가 없다. `ProgressHeader` 좌측 112px 슬롯에
넣고 **모바일에도 함께 노출**한다(모바일도 되돌아갈 수단이 OS 제스처뿐인 건 마찬가지다).

딸린 규칙: 뒤로 버튼은 위저드 안에 돌아갈 단계가 있을 때만 보인다. 히스토리 state 의 `obDepth`
로 판단한다 — 재가입은 `initialStep = 5` 로 중간에서 시작하므로 step 값만으로는 진입 지점을 알 수
없고, 깊이 0 에서 뒤로를 누르면 앱 밖으로 나간다.

투두의 시간 선택기(`TimeWheels`)는 클릭 버튼 + 직접 입력이라 이 문제가 없다 — 이번 변경 대상이
아니다. 자세한 내용은 `docs/product/04-organization-invitations.md`.

## 2026-09-09 타이포그래피 — 한글·가나에는 라틴 자간을 씌우지 않는다

「회원가입 글씨체가 어색하다」는 제보. 원인이 셋이었고 성격이 서로 달랐다.

**① `--font-sans` / `--font-mono` 토큰에 CJK 가 빠져 있었다.** `body` 스택은
Geist → Noto Sans KR 로 맞아 있었는데, Tailwind 토큰은 Geist 다음이 곧바로 `ui-sans-serif` 였다.
`font-sans` 유틸을 쓴 곳만 한글이 시스템 글꼴로 떨어져 **한 화면 안에서 글씨체가 달라 보였다** —
회원가입 초대코드 입력의 `placeholder:font-sans` 가 그 예다. 토큰은 항상 `body` 스택과 같은 순서를
유지한다.

**② 라틴 기준 자간이 한글에 그대로 걸려 있었다.** 아이브로우 `uppercase tracking-[0.13em]` 은
「설 정  완 료」처럼 흩어지고, 제목 `tracking-[-0.03em]` 은 답답해진다. 한글·가나는 이미 정사각
자간으로 설계된 글자라 letter-spacing 을 더하거나 빼면 바로 어색해진다.

**결정: 라틴 자간은 그대로 두고 CJK 일 때만 덮는다.** 공용 유틸 `.t-eyebrow` / `.t-display` 를
두고 `:lang(ko)` / `:lang(ja)` 에서만 보정한다. 영어 화면의 디자인 의도는 손대지 않는다.
0 으로 완전히 되돌리지 않고 제목에 `-0.005em` 을 남긴 것은, 큰 제목이 헐거워 보이지 않게 하기
위해서다.

**③ `<html lang>` 이 세션이 없으면 무조건 "ko" 였다.** 로그인·회원가입은 로그아웃 상태로 보는
화면이라, 일본어/영어로 보고 있어도 문서 언어는 한국어라고 선언돼 있었다. 스크린리더가 영어를
한국어 음성으로 읽는 접근성 버그이자, ② 의 `:lang()` 보정이 엉뚱하게 걸리는 원인이다.
세션 → 쿠키 → Accept-Language → ko 로 고쳤고, 레이아웃이 읽을 수 없는 `?lang=` 구간은
**로그인·온보딩이 자기 루트에 `lang` 을 직접 달아** 메운다.

**남겨 둔 것: 첫 진입 폰트 전환.** Noto Sans KR/JP 는 `preload: false` 라 캐시 없는 첫 방문에서
한글이 잠깐 시스템 글꼴로 보인다. 회원가입이 대개 첫 화면이라 가장 잘 보이는 자리지만,
`preload: true` 는 모든 라우트가 큰 CJK 폰트를 선로딩하게 만든다 — 성능과의 교환이라 별도 결정으로
남긴다.

## 2026-09-09 (2차) 한글 타이포 — 회원가입만이 아니라 앱 전체였다

회원가입 화면을 고친 뒤 「다른 데는 없냐」는 확인 요청으로 전수 조사를 돌렸다. 같은 결함이
앱 전반에 있었다 — 손으로 쓴 CSS 47개 규칙(13개 파일), Tailwind 인라인 124곳, 폰트 토큰 9곳.

**결정 1: Tailwind 쪽은 클래스를 124번 달지 않는다.** 결함의 «형태»를 선택자로 쓴다 —
`uppercase` 와 양수 자간을 함께 가진 요소(`.uppercase[class*="tracking-[0."]`). 음수 자간은
`tracking-[-0.` 이라 자동으로 빠진다. 규칙 한 벌로 124곳이 덮이고, 새로 추가되는 코드도 자동으로
포함된다. 담기는 문구를 전수 확인해 하드코딩된 라틴 라벨이 0건임을 근거로 삼았다.

**결정 2: 앱 전체 규칙에서는 `text-transform` 을 건드리지 않는다.** 자간만 되돌린다.
한글에 uppercase 는 어차피 효과가 없고, 같은 규칙을 공유하는 라틴 전용 라벨의 대문자를 잃지
않는다 — 이득 없이 회귀 위험만 있는 변경은 하지 않는다. 회원가입 화면은 문구가 순수 한글임을
확인했으므로 그곳만 예외로 함께 끈다.

**결정 3: 손으로 쓴 CSS 의 오버라이드는 각 파일 끝에 둔다.** globals.css 한 곳에 모으면
짧아지지만, 선택자 이름이 바뀔 때 오버라이드가 조용히 죽는다. 같은 파일에 있으면 함께 눈에 띈다.

**함께 나온 것: `--mono` 가 6곳에 따로 정의돼 있었고 그중 한 곳만 CJK 를 포함했다.** 숫자 전용
토큰처럼 보였지만 실제로는 한글이 섞여 든다 — 연차 이력의 `3일` 에서 `일` 만 시스템 고정폭이었다.
여섯 정의를 하나로 통일했다. `--font` 는 더 나빴다: 정의가 공지 콘솔에만 있고 그 값에 Geist 도
Noto 도 없어서, 같은 단위 라벨이 페이지마다 다른 글꼴로 보였다(다른 페이지에서는 변수가 없어
body 를 상속해 «우연히» 맞고 있었다).

토큰이 여러 곳에 복제되면 그중 하나만 고쳐진 채로 오래 간다 — 이번 `--mono` 가 정확히 그랬다.

## 2026-09-09 (3차) 투두는 개인 공간 — 가시 범위를 RLS 밖에서도 강제한다

콘솔 「오늘」 목록에 다른 사용자가 혼자 만든 개인 투두가 「작성자 → 제목」으로 섞여 있다는 지적에서
시작했다. 접두사 렌더링은 정상이었다 — **그 작업이 목록에 들어온 것 자체가 결함**이었다.

원인: 가시 범위 판정을 전부 RLS 에 맡겼는데, `tasks` 의 select 정책은 `is_platform_admin()` 이면
참여자 조건을 통째로 건너뛴다. 조직에 활성 플랫폼 관리자가 한 명 있었고, 그 계정이 조직 전원의
개인 투두를 오늘·내일·관리함·완료·기록에서 보고 있었다.

**결정 1: 투두는 개인 업무의 공간이다.** 남의 작업이 내 화면에 들어오는 경로는 셋뿐 — 공유받은
작업, 받은 지시, 내가 속한 프로젝트의 작업. 그 밖에는 **역할과 무관하게** 보이지 않는다. 조직을
관리할 권한과 남의 개인 할 일을 들여다볼 권한은 다른 것이다.

**결정 2: RLS 의 관리자 예외는 유지하고, 앱에서 이중으로 막는다.** 정책을 좁히면 조직 간
운영·복구 경로가 함께 막힌다. 대신 투두 읽기 경로에 `getTaskScope(session)` 게이트를 두었다 —
조직 격리와 같은 원칙(서버에서 강제, UI 필터에 의존하지 않음).

**결정 3: 게이트는 참여자 행을 근거로 삼는다.** 역할 목록이 아니라 `task_participants` /
`project_participants` 다. 역할 기반으로 하면 새 역할이 생길 때마다 다시 판단해야 하고, 판단을
빠뜨리는 쪽이 «더 많이 보이는» 방향으로 실패한다. 전제(모든 생성 경로가 작성자 참여자 행을 함께
넣는다)는 적용 전 실측으로 확인했다 — 누락 0건.

**팀 전체 조망이 필요해지면 별도 화면으로 설계한다.** 개인 투두 목록에 남의 할 일을 섞는 방식으로는
만들지 않는다.

## 2026-09-09 (4차) 완료·기록에서 보낸 지시를 뺀다 — 지시자는 작업자가 아니다

「내가 지시 보낸 사용자가 완료하면 내 완료기록에 뜬다」는 지적. 2026-07-29 에 "완료·기록 리스트는
팀 완료를 기록으로 계속 표시"로 결정해 둔 것이 지시에서는 틀렸다.

**결정: 지시에 한해 철회한다.** 보낸 지시를 대상자가 완료한 건은 지시자의 완료·기록에서 뺀다.
지시자는 「지시 › 보낸 지시」에서 진행 상황을 본다.

**공유(공동 작업)의 팀 완료는 유지한다.** 공유는 둘 다 작업자이므로 원래 결정이 계속 맞다.
지시는 «시킨 사람 / 하는 사람»이 갈리는 유일한 관계라, 여기만 예외다.

**내가 직접 완료한 보낸 지시는 남긴다.** 판정을 작업 종류가 아니라
`sentInstr(task, me) && completion.byUserId !== me` 로 둔 이유다 — 내가 한 일을 숨기지 않는다.

「보낸 지시는 대상자의 일정」이라는 규칙(`myOwn`)은 이미 오늘·내일·관리함·캘린더에 적용돼 있었다.
완료·기록만 그 규칙을 통과시키지 않은, **기존 규칙의 누락**이었다.
