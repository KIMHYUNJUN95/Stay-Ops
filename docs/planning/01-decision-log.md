...
채용 사이트 어드민이 Firebase Auth 를 쓰지 않는 것 — 비밀번호가 번들 안 상수라 로그인한 사용자가
없고, 그래서 그 화면이 목록을 보려면 규칙이 공개 읽기를 허용해야 했다. 그 비밀번호는 화면만
가리고 데이터는 가리지 못했다.

「주소를 누가 아느냐」는 방어가 되지 않는다. `projectId` 와 컬렉션 이름이 사이트 번들에 문자열로
들어 있고, 개발자도구 Network 탭에도 그대로 보인다. 공개 Firestore 는 자동 스캐너가 훑는 흔한
설정 실수다.

**순서를 지켜 무중단으로 했다.** ① 서비스 계정으로도 읽을 수 있게 만든다(자격증명이 없으면 예전
...
안 되지」를 구분할 수 없어서 `authState` 를 추가했다 — not_configured / invalid_json /
missing_fields / configured. 원인을 좁힐 수 없는 진단은 그 자체로 결함이다.

**firebase-admin 을 넣지 않았다.** 당겨오기가 이미 REST 라 토큰만 붙이면 되고, SDK 는 의존성과
콜드스타트를 늘린다. JWT 서명은 Node 기본 crypto 로 끝난다.

Status: 완료 (2026-09-11). 문서: `docs/product/30-recruit-workflow.md` → 「닫힌 것」,
`docs/engineering/07-environment-setup.md` → Firestore 서비스 계정.

## 2026-09-11 모바일 뒤로가기 — 문서가 없는 구현을 설명하고 있었다

**사용자 질문에서 드러났다.** 「화면을 옆으로 쓸어서 뒤로가기 못하지?」 — 문서를 보고 「구현돼
있다」고 답했다가, 코드를 확인하니 **없었다.**

`16-mobile-navigation.md` 는 2026-06-15 부터 「`MobileShell` 이 `<main>` 에서 `handleSwipeStart` /
`handleSwipeMove` / `handleSwipeEnd` 로 엣지 스와이프를 처리하고, ~64px 을 넘기면 `router.back()`,
오른쪽 엣지는 `router.forward()`」라고 적고 있었다. 2026-06-22 에는 `goBack()` 의 되돌림 방지까지
덧붙였다.

**전수 확인 결과 `handleSwipe*` · `edgeDx` · `edgeRawDxRef` · 셸의 `goBack` 중 어느 것도 없다.**
같은 시기에 적힌 PTR 서술(`ptrEligibleRef` 등)은 지금도 유효하다 — 엣지 백 부분만 제거되고 문서가
남았다. `mobile-calendar-view.tsx` 에 「왼쪽 가장자리에서 시작한 가로 스크롤이 `router.back()` 을
발동시키곤 했다」는 **과거형** 주석이 있어, 가로 스크롤 충돌 때문에 뺀 것으로 보인다.

**결정: 다시 구현하지 않는다 (사용자 판단).** 실제 기기에서는 OS·브라우저가 엣지 스와이프를 기본
제공한다 — 사용자가 확인했다. 네이티브 제스처 위에 자체 구현을 얹으면 **두 번 뒤로 가거나** 가로
스크롤과 싸운다. 이 저장소가 이미 겪은 실패다.

뒤로가기 버튼을 없앤 2026-06-15 결정은 **유지**한다. 근거가 「우리가 구현한 엣지 스와이프」에서
「기기가 기본 제공하는 제스처」로 바뀔 뿐이다.

**확인되지 않은 경우.** 홈 화면에 설치한 standalone(`display: standalone`)에서도 제스처가 도는지는
확인하지 않았다. 이 앱은 `start_url` 이 `/mobile` 인 설치형 PWA 가 전제라 그 경우가 실사용이다.
안 된다면 그 경우에만 뒤로 갈 수단이 없다 — 실측 후 문서를 갱신할 것.

**교훈.** 문서가 「구현돼 있다」고 단정하면 다음 사람은 그 코드를 찾거나 고치려 든다 — 실제로 내가
그랬다. 기능을 뺄 때 그 기능을 설명하던 문서를 함께 되돌려야 한다. 이 저장소의 완료 규칙
(코드와 문서는 한 단위)이 정확히 이걸 막으려는 것이다.

Status: 문서 정정 완료 (2026-09-11). 코드 변경 없음.
문서: `docs/product/16-mobile-navigation.md` — 과거 서술은 `<details>` 로 남겨 이력을 지우지 않았다.
...
[33](../product/33-calendar-write-features.md) ·
[34](../product/34-metrics-and-automation.md)

확정한 것:

- **이전 범위 4개** — 캘린더(전 기능) · 가동률 · 매출 · 자동화(구글·슬랙·노션). 그 외는 안 가져온다
- **모바일이 설계의 출발점이다.** 저쪽은 PWA 이지만 모바일이 사실상 안 된다. 이쪽에서는
  **읽기·쓰기 모두 모바일에서** 가능해야 한다. 그래서 저쪽의 마우스 드래그 조작을 그대로 옮기지
  않고 「대상 선택 → 값 입력 → 확인」 흐름으로 다시 설계한다
- **병행 기간에는 데이터를 가져오지 않는다.** 설계·구현만 해 두고, 개발 완료 + 데이터 동기화
  확인 후에 전환한다. 이 원칙이 「두 웹이 같은 Beds24 에 써서 덮어씀」과 「리포트 이중 발송」
  두 위험을 원천적으로 없앤다
- **전환 순서: 읽기 → 자동화 → 캘린더(쓰기).** 쓰기가 마지막인 이유는 틀렸을 때 에어비앤비·
  부킹닷컴에 그대로 나가기 때문이다. 읽기로 데이터가 맞다는 것을 먼저 확인한다
- **별도의 운영 관리자 영역**을 만든다. 기존 콘솔에 섞지 않는다 — 기존 캘린더는 읽기 전용이고
...
- **캘린더 기간은 분리한다**(2026-09-16). 과거 예약을 백필해도 **기존 캘린더는 그대로 3개월**로
  둔다 — 현장이 매일 쓰는 화면이고 그 제한이 그 화면의 전제다. 넓은 기간은 **운영 관리자
  캘린더**(12개월 이상)가 맡는다. 두 캘린더를 분리한 이유와 같다: 보는 사람도, 목적도, 필요한
  범위도 다르다.
...
같은 값을 매체에 따라 다르게 다루는 것이므로 근거를 남긴다 — 내보내기가 빼는 이유는 「한번 나간
파일은 회수할 수 없다」였고, 인쇄물은 손에서 손으로만 움직이며 종이 지원서를 대체하는 물건이라
면접장에서 본인 확인·연락에 바로 쓰인다. 화면 기본 마스킹은 그대로 둔다(어깨너머 노출 방지).

**결정 2: 한 번에 한 건.** 목록 선택분 일괄 인쇄는 넣지 않았다. 검토는 한 사람씩 하는 일이고,
일괄은 「몇 명 뽑아 공유」에 해당하는데 그건 이미 내보내기가 한다.

**결정 3: 인쇄는 패널 헤더의 보조 동작 자리(`.panel__tool`)에 둔다.** 하단 액션 바의
왼쪽(파괴)·오른쪽(진행) 자리는 반복 처리 중 오조작을 막으려고 고정돼 있다. 상태를 바꾸지 않는
동작을 그 사이에 끼우면 그 고정이 깨진다. 이 자리를 공용 프리미티브로 정의해 다른 콘솔 패널도
같은 위치를 쓰게 했다.

**결정 4: 새 라우트를 만들지 않는다.** 화면에서 숨어 있는 A4 시트 + `@media print` 로 간다
(`休暇届` 와 같은 방식). 인쇄 페이지로 이동시키면 「인쇄」 한 번에 패널이 닫히고 목록 상태를 잃는다.

## 2026-09-24 (2차) 채용 사이트는 지원자용만 — 어드민·실시간 상담 폐지 (실행 완료)

**결정 1: 관리자 포털을 통째로 없앤다.** 지원서 탭은 2026-09-11 Firestore 읽기 차단 이후 403 만
...
죽어 있는 것을 규칙 작업 중에 발견했다. 「하는 김에」 여는 것은 별개의 결정이고, 열면 누구나
카운터를 임의의 수로 쓸 수 있다. 목적(= `chat_rooms` 차단)만 하고 나머지는 현재 동작을 그대로
재현했다.
...
- React 19.2 · Tailwind 4 · TypeScript 5.9 — **메이저가 이미 일치한다.** 스타일은 거의 그대로 옮는다.
- **공개 페이지 구조가 이미 준비돼 있다.** `middleware.ts` 는 보호 경로를 접두사 셋
  (`/admin` · `/mobile` · `/account`)으로만 판정하고 나머지는 그대로 통과시킨다. `/careers/*` 같은
  경로는 **미들웨어를 고치지 않아도** 공개로 동작한다. 여기가 제일 큰 걸림돌일 줄 알았는데 아니었다.
- 전환이 필요한 것: Vite→Next(App Router), react-router→App Router, Firebase→Supabase,
  의존성 3개 추가(framer-motion · gsap · react-player).

### 합치면 사라지는 것 (이전의 진짜 이득)

지금 복잡도의 상당 부분이 **두 시스템을 잇는 배관**이다. 지원 폼이 Supabase 에 직접 쓰면 통째로
없어진다 — pull 엔드포인트 · GitHub Actions 크론 · `recruit_sync_state` · 60초 창 · 하루 1회 전량
훑기 · 502 알림 · 이력서 복사 파이프라인 · **서비스 계정 키 운영 의존성** · Firestore 규칙 관리 ·
Firebase 프로젝트 자체.
...
- **번들 무게** — gsap · framer-motion · react-player 는 공개 페이지에만 쓰이므로 라우트 분리 필요.
