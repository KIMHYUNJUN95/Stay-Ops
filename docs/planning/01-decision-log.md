...
## 2026-08-04 — 이메일(Gmail) 연동: 검토 후 **기각**

Gmail 을 StayOps 안에서 처리하는 기능을 기획했다가 **만들지 않기로 했다.** 내세웠던 가치가
전부 더 싼 방법으로 이미 해결되거나 해결 가능했다:

- **예약 알림 정리** — Gmail 필터 "받은편지함 건너뛰기"로 끝난다(코드 0줄). 받은편지함 8통 중
  7통이 이것이었다.
- **웹훅 누락 탐지** — **이미 있다.** `src/app/api/beds24/reconcile` 이 매일 Cron 으로 Beds24
  `/bookings` API 를 다시 당겨 누락을 치유한다. 메일 제목 대조보다 API 직접 대조가 더 정확하다.
- **답장·삭제·폴더 정리** — Gmail 이 더 잘한다. 우리가 만들면 느리고 기능은 적다.

비용은 몇 주 구현 + **영구 유지보수**다(OAuth 토큰 수명, HTML 정화/XSS, 스레드 헤더, 첨부, 라벨,
검색, 오류 상태 × ko/ja/en × 모바일·콘솔, 구글 API 변경 대응). 4명 규모에서 "Gmail 탭을 안 열어도
되는 것"의 값으로는 비싸다.

**남는 유일한 고유 가치는 청구서 미도착 감지였다** — 매월 오던 청구서가 안 온 것은 받은편지함에서
*없는 메일*이라 절대 눈에 띄지 않고, 이메일이 유일한 소스다. 필요해지면 그것만 좁게 만든다
(읽기 권한으로 "이번 달 이 발신자 메일이 왔는가"만 확인하면 되므로 메일 클라이언트가 필요 없다).

**교훈:** 기능을 기획하기 전에 "그 가치가 이미 다른 곳에 있는가"를 먼저 확인한다. reconcile
엔드포인트는 조금만 찾아봤으면 나왔다.
