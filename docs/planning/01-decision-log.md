...
## 2026-08-04 — 이메일(Gmail) 연동 방향

**결정 3건** (상세: `docs/product/29-email-integration.md`)

1. **Google Workspace 계정만 대상으로 한다.** Gmail 의 읽기·발신·라벨 변경은 구글의 restricted
   scope 라, 외부 앱으로 게시하면 OAuth 심사 + CASA Tier 2 보안 평가(연 $500~4,500, 수 주)를
   매년 받아야 한다. 심사 없이 테스트 모드로 쓰면 refresh token 이 7일마다 만료되어 실무에 못
   쓴다. Workspace 도메인이면 **내부 앱**으로 게시해 둘 다 면제된다. **이 전제가 깨지면 기획
   전체를 다시 본다.**

2. **메일 본문을 저장하지 않는다.** 열 때마다 Gmail API 로 가져온다. 본문에는 투숙객 이름·연락처가
   섞이고, 우리 DB 에 쌓는 순간 보존 기간·삭제 요청 대응·RLS 를 새로 정의해야 한다. **보관하지
   않는다는 사실 자체가 가장 강한 방어다.** 대가로 우리 쪽 전문 검색을 포기하고 Gmail 검색 API 에
   위임한다.

3. **전체 권한(`https://mail.google.com/`)을 쓰지 않는다.** `gmail.readonly` + `modify` + `send`
   로 충분하다. 전체 권한이 추가로 주는 것은 사실상 영구 삭제뿐인데, 사용자가 "삭제"라고 할 때
   기대하는 동작은 휴지통 이동이다. 운영 메일을 StayOps 에서 복구 불가능하게 지울 이유가 없다.

**로그인용 구글 OAuth 와 분리한다.** 현재 로그인은 인증만 하고 Gmail 권한도 오프라인 토큰도 받지
않는다. 거기에 scope 를 얹으면 **로그인하는 모든 사람에게 메일 권한을 요구**하게 된다. 메일 연동은
설정 화면의 별도 연결 흐름으로 둔다.

**공용·개인 메일함을 둘 다 지원하되 한 번에 열지 않는다.** 모델은 처음부터 `scope`(shared |
personal)로 담고, v1 은 공용만 노출한다. 개인 메일함은 **관리자도 열람 불가**를 서버에서 강제해야
하는 노무·프라이버시 경계가 있어, v1 에 끼워 넣으면 검증할 표면이 두 배가 된다.
