...
- **Bug Report / Problem Report** — planning docs started on 2026-06-25. This is a new
  **StayOps product/system issue reporting** module, distinct from Maintenance (real-world property
  issues) and Staff Suggestions (person-directed feedback). Current state is **docs-only draft**:
  Product `25` + Tech-design `13` created, with first-slice assumptions and open questions recorded.
  **2026-06-25 clarification:** scope is explicitly **StayOps app bug / product issue reporting**,
  and the feature UI design will be provided by the user separately.
...
- **Onboarding membership-only path unified into the wizard (2026-06-24).** The post-profile `needs_membership` route no longer falls back to the older join-only screen. It now reuses the same onboarding wizard and opens directly at **step 4 (invite code)** with the saved profile values preloaded for the later review/submit step. For this membership-only path, `코드 없이 나중에 입력` is disabled and an explicit **로그인 화면으로 돌아가기** action was added so a stuck test account can exit cleanly instead of being trapped on the join screen. Docs/product sync: Organization + Invitations `04`.
