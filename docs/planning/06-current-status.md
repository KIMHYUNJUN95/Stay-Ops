...
- Legacy email magic-link login still exists in the current implementation, but it is **no longer the planned auth model** after the 2026-06-18 auth reset.
- `/auth/login` visual redesign completed on 2026-05-21 and final-polished with the announcement surfaces. The current screen styling remains usable, but its auth-method composition is now partially outdated because the next auth pass will remove magic-link and replace it with the new `Google + email/password` policy.
...
- **Entry-routing policy changed (2026-06-18 docs):** the root-level "dashboard vs mobile" choice
  screen is no longer allowed in the product direction. Target behavior is:
  - desktop/PC → admin dashboard automatically
  - mobile/tablet → `/mobile` automatically
  The current desktop root still renders `DevEntry`, so this remains a follow-up implementation task.
...
## 2026-06-18 Auth / Signup Planning Reset (documentation baseline only)

The login/onboarding policy was re-planned before the next auth implementation pass. This is a
**documented target policy**, not a completed implementation yet.

Confirmed target policy:

- Auth methods: `Google` + `email/password`
- `email magic-link` removed from the product plan
- Google profile data is authentication-only; StayOps must not auto-fill name/phone from Google
- Required onboarding before app access:
  - name
  - date of birth
  - phone number
  - preferred language
...
- Signup role categories:
  - Part-time Staff
  - Office Staff
  - Field Staff
  - Part-time Staff (Manager)
  - Owner
- `Owner` invite codes are one-time use
- Other invite codes use the shared rule: `3 months / max 100 joins`
- One account can belong to multiple organizations
- Login should return to the last-used organization
- Organization creation is not fully public; the creator of the first organization becomes its first owner
- Until dashboard tooling exists, initial organization / owner / invite-code bootstrap is manual DB work
- Root entry must no longer expose a manual "dashboard / mobile version" chooser:
  - desktop/PC should auto-enter dashboard
  - mobile/tablet should auto-enter `/mobile`
  - future dashboard-side mobile access belongs inside dashboard UI, not on the root landing screen

Implementation note:

- The existing auth/onboarding implementation still reflects earlier assumptions in several places
  (for example older invite/onboarding semantics and the pre-reset auth model).
- The existing root entry still contains `DevEntry` on desktop, which now conflicts with the updated
  entry-routing policy.
- Code, schema, and auth UI must be updated in a later implementation pass to match this new policy.
- Treat the docs above as the new source-of-truth target for that follow-up work.
