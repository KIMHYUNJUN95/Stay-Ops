...
4. **Board (자유 게시판)** — **기획 확정 (2026-06-25).** 기존 product `20` 스켈레톤을 대체해 전면 재기획 완료. 전 직원(아르바이트 포함)이 작성하는 공개 자유 게시판으로 확정. 공지사항(Announcements)과 완전히 분리된 별도 메뉴. 핵심 기능: 글 작성(제목 선택·본문 필수·이미지 최대 5장·자유 태그), 이모지 반응(👍❤️😂😮😢), 댓글(이미지 최대 3장), 고정(관리자), 읽음 추적. DB 테이블 4개 설계 완료(`board_posts`, `board_post_reads`, `board_comments`, `board_reactions`). RLS·서버 액션·i18n 키·알림 타입 설계 완료. **다음 단계:** UI/UX 디자인(사용자 진행) → 구현. 전체 기획: `docs/product/23-board-workflow.md`. See decision log 2026-06-25.
