...
2. **Personal Todo / Shared Task Inbox** — **First slice implemented (2026-06-10), hardened 2026-06-11.** Mobile Todo/Shared Task is live in the working tree under `/mobile/tasks/*` (side-menu entry `tasks`): all six views (Today/Inbox/My/Sent/Completed/Calendar), quick add + detailed create/edit, task detail with unified update log (text + photos), multi-select sharing, common shared status, and full participant management (author removes any participant, anyone removes self, author-leave deletes for all). Tables `tasks`, `task_participants`, `task_updates` (migration `202606100003_todo_tasks.sql`); photos reuse the `request-images` bucket (`task-images`/`task-update-images`). Hardening pass: fail-safe creation (orphan-row rollback), author-editable task photos, update-log photo upload, removal of the misleading Sent "new update" dot, localized calendar weekday headers, and the Inbox decision (detailed create = organized, not Inbox). Second cut: fail-safe re-sharing (no false shared state/notifications on participant-insert failure), inline localized error on update-log photo-upload failure, and Storage hard-delete of removed task-level photos (org/`task-images`-scoped, server-side). Search/filter first slice: client-side title+author text search and anchor-date single/range filter across the list views (not Calendar), with a clear active-filter indicator and a distinct no-match vs empty state. Calendar upgrade: month-to-month navigation (+ Today reset), a month-scoped agenda grouped by day (replacing the flat upcoming list), persistent selected-date emphasis with a count in the day sheet, and clear empty states — all on the same anchor-date semantics, still client-side and separate from the reservation calendar. Production-polish pass: the grid is framed in a white surface card with the legend below it, calm today-vs-selected cell states on an even marker rhythm, a height-capped scrollable day sheet, intentional empty blocks, and a soft accent "add on this date" CTA (visual only, no behavior change). Time-input pass: the create/edit Time section now has an All-day toggle + direct time picker + quick chips + clear-to-all-day, persistence is normalized through one shared `normalizeTaskDateTime()` helper (time_label is authoritative, inputs validated), and the rule is documented in Tech-design `09` "Time handling". Time-consistency fix: all-day due tasks render date-only in detail (no misleading "00:00"), and entering a time with no scheduled/due date is now rejected with a clear localized error (form block + server guard in both create and edit) instead of silently dropping the time. Recurrence pass: recurrence is clarified as a display-only reminder label (no instance generation) with an inline hint, None+5 standard rules are selectable and clearable, and `custom` is narrowed to a recognized-but-not-creatable bucket (read-only chip surfaced only for legacy values, never newly assignable) — create/edit persistence stays identical and fails closed to null. Enforcement fix: the `custom` round-trip rule is now enforced server-side via a shared `resolveRecurrenceRule(submitted, previousRule)` helper (create can't assign `custom`; a non-custom task can't become `custom`; only an existing `custom` task round-trips), so the server contract matches the UI/docs and a crafted request can't bypass it. Notification expansion: added `task_due_soon` + `task_overdue` types (migration `202606110001_task_reminder_notifications.sql`) emitted by a daily CRON_SECRET-guarded `/api/tasks/reminders` cron (08:00 JST) — due-soon = active task due today (Tokyo), overdue = active task due before today, one deduped reminder per task per recipient (no escalation/spam, no scheduler engine); event-driven shared/update-log(`task_updated` event `note`)/completed already existed and stay coherent. **Activation status (2026-06-11): the enum migration `202606110001_task_reminder_notifications.sql` is APPLIED to the linked Supabase project (`public.notification_type` now has all six values incl. `task_due_soon`/`task_overdue`, recorded as remote migration `task_reminder_notifications`). REMAINING manual step: set `CRON_SECRET` in the Vercel project env — `/api/tasks/reminders` returns 404 until it is set, and Vercel Cron needs it to authorize the daily run. The remote Vercel env value cannot be verified from the dev environment.** Create-flow clarity pass: the Quick Add sheet now plainly contrasts **Save to Inbox** (fast title-only capture, `is_inbox=true`) vs **Full create** (the organized path → `/mobile/tasks/new`, `is_inbox=false`, where dates/share/repeat are set), the typed title carries over (`?title=`) so escalating doesn't lose input, and helper copy + a new-page subtitle state the destinations — wording only, persistence unchanged; Calendar date-prefill still works. Quick-add UI polish (2026-06-11): the Quick Add launcher is now a circular `+` FAB and the capture sheet is a bottom sheet with slide in/out animation, warm-token field/close-button styling, Esc-to-close, and is body-portaled (together with the FAB) so both stay viewport-fixed over the bottom tab bar. Detailed create/edit redesign (same pass): the old fixed bottom Back/Save action bar (which drifted mid-content because the shared scroll container's resting transform traps `position: fixed`) was removed entirely; Save is now a small top-right navy button in a scrolling page header (back + title + Save) that the form owns so it stays a native submit with `isPending`, and the "More" disclosure was restyled to a hairline-topped row with a rotating chevron and a bold lead + muted hint. Header ownership moved from `new/page.tsx` and `[id]/edit/page.tsx` into `TaskCreateForm` (new `backHref` / `headerTitle` props); the mobile shell was left untouched. The Share-target picker (`SharePicker`, used by both the create form and the task detail view) got the same bottom-sheet treatment — body-portaled with a slide up/down transition and Esc-to-close (the parent keeps it mounted until the exit transition plays), so it clears the bottom tab bar instead of being clipped behind it. List quick-complete + undo (2026-06-11): tapping a task card's left circle now completes it in place (Gmail-style) — the card is optimistically hidden and a small "Marked complete · Undo" snackbar slides up above the tab bar for ~4s; Undo restores the card with no server round-trip (and no completed/reopened notification churn), while letting the window lapse commits via a new list-context `completeTaskInList(taskId)` server action that `revalidatePath`s the list instead of redirecting to the detail page. A still-pending completion is flushed on unmount so it isn't lost on navigation. The swipe-to-complete action shares the same flow; `TaskCard` gained an optional `onQuickComplete` callback (falls back to the redirecting server-action form when absent). New i18n: `completedToast` / `undo` (ko/ja/en). Tab counts + completed grouping (2026-06-11): each list-view chip tab shows a small count badge (toned for active vs. inactive chips, hidden at zero, sharing the visible-task counts so optimistically-completed cards drop out instantly), and the Completed view is now grouped by completion date (Tokyo) under "6월 11일 · 오늘 · 목요일 · N"-style dividers (bold date + today/yesterday relative label + weekday + count), newest day and newest task first. New i18n: `yesterdayLabel` (ko/ja/en). Tab model redefinition (2026-06-11, supersedes the earlier Inbox-vs-organized split): **Today** unchanged (overdue + today); **Inbox → 관리함 / Manage / 管理箱** now shows *every active todo* as a single management list (newest first), no longer filtered by `is_inbox`; **My (내 task)** now shows *all active todos except Today's*, grouped by date like the Completed view (earliest/overdue first, undated last); **Sent / Completed / Calendar** unchanged. The `is_inbox` column still exists and quick-add still sets it, but it no longer gates any view, so the now-redundant swipe-to-Inbox card action was removed (swipe is Today + Complete). Quick-add copy reworded Inbox → 보관함/Archive (`viewInbox`, `inboxHint`, `inboxEmptyTitle/Sub`, `quickAddSub`, `quickAddSave`, `todayEmptySub`); `swipeInbox` / `detailedCreateHint` are now unused. Calendar review: logic verified (month nav, Today reset, agenda, weekday colors) and its day sheet was given the same body-portal + slide in/out fix so it clears the bottom tab bar. NOTE: product/IA docs (`18-*` task workflow, `16-mobile-navigation`) still describe the old Inbox semantics and should be reconciled next. Long-press + multi-select (2026-06-11): long-pressing a task card (≈480ms hold, also desktop right-click) opens a body-portaled context sheet with Edit / Select / Delete (Edit + Delete shown only to the task author; haptic on fire, movement/scroll cancels the press, the follow-up tap is suppressed). "Select" enters a multi-select mode where the chip tabs are replaced by a selection bar (count + Select-all/Clear + cancel), each card shows a checkbox, swipe/navigation are disabled, the quick-add FAB hides, and a body-portaled bottom bar offers batch Delete with a centered confirm modal. Deletion runs through a new `deleteTasksInList(taskIds[])` server action whose `created_by_user_id` filter is the authorization boundary (selecting tasks shared to you that you don't own leaves them untouched); rows are optimistically hidden (reusing the quick-complete `pendingDone` set) and `revalidatePath`'d. The now-defunct swipe-to-Inbox action was already removed. New i18n: `actionEdit` / `actionSelect` / `selectAllLabel` / `selectClear` / `selectedCountLabel` / `bulkDeleteConfirmTitle` / `bulkDeleteConfirmBody` (ko/ja/en). Swipe + picker polish (2026-06-11): an opened swipe row now closes when you tap anywhere outside it (or tap the open card); the reveal buttons were redesigned to calm on-theme chips (muted "오늘로" + navy "완료") with elastic drag + spring snap, "오늘로" is hidden in the Today view (driven by a `showMoveToday` prop, not the task date). The detailed create/edit form replaced the native `<input type=date>` / `<input type=time>` with custom in-form dropdowns (`src/components/tasks/date-time-fields.tsx`): `MiniCalendar` (full-width month grid, no side overflow) for 예정일/마감일, and `TimeWheels` (오전/오후 + 시 + 분 scroll columns **plus** direct HH:MM entry) for the time — `TaskCreateForm` gained a `locale` prop. New i18n: `amLabel` / `pmLabel` / `hourLabel` / `minuteLabel` / `timeManualHint` / `pickDate` / `clearValue` (ko/ja/en). Quick-add "오늘에 추가" button (2026-06-12): the Quick Add sheet now has a third action button below the two existing ones. Tapping **오늘에 추가** (Sun icon) creates the task with `scheduled_date = today (Tokyo)` and `is_inbox: false` via a new `quickCreateTodayTask` server action, then redirects to `?view=today` so the card is immediately visible in the Today tab. The existing "관리함에 저장" path (`is_inbox: true`) is unchanged. New i18n: `quickAddToday` (ko/ja/en). See Product `18` + Tech-design `09` "Hardening pass".
