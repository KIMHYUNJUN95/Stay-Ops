...
Standalone visual follow-up: after hiding that shared chrome, a remaining artifact was the **bright top-right and bottom-right blocks** in the exposed sliver. Root cause: the standalone scrim logic was still preserving the old `safe-area + 64px header` and bottom-tab clear bands even though those surfaces were now hidden during `sidebarOpen`. Fix: when the sidebar is open, standalone mode now preserves only the literal `env(safe-area-inset-top/bottom)` rows, not the old header/tab-bar clear zones. The status bar remains protected, but the exposed sliver now dims continuously from just below the safe area, which removes the "screen looks cut into three horizontal bands" feel.
