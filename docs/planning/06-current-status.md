...
- **이메일(Gmail) 연동 기획 착수 (2026-08-04).** 운영 메일을 StayOps 안에서 확인·답장·정리하고,
  나아가 **메일을 컴플레인·유지보수·투두로 전환**하는 것이 목표다(그게 이 기능의 존재 이유 —
  목록·답장만 옮기면 Gmail 을 더 불편하게 만드는 데 그친다). 확정: Workspace 계정 한정(심사·CASA
  면제), 본문 미저장(실시간 조회), 전체 권한 대신 readonly+modify+send. 로그인용 OAuth 와 분리.
  v1 은 공용 메일함만, 개인 메일함은 v2. **구현 착수 전** — 실물 OAuth 확인과 운영 메일 기준
  결정이 선행되어야 한다. 기준 문서: Product `29`, decision log.
