...
### 다음에 할 일 ②  답이 필요한 결정 (개발자가 못 정한다)

1. **블록 라이브 검증을 해도 되는가.** 먼 날짜에 실제로 걸었다 바로 푸는 것. 거는 순간
   그 방이 **채널에서 판매 정지**된다. 되읽기로 확인 후 즉시 해제하므로 노출은 수 초.
2. **다카다노바바 방 이름.** Beds24 의 roomType 이름이 `2·3·4…9` 다(`201호`가 아니다).
   그래서 `401_2`(556719)가 본 유닛 `4`(513700)와 한 행으로 안 합쳐지고, 캘린더에도 `4` 로
   보인다. **2026-09-25 에 「어느 쪽이 진짜인가」로 실제 혼선이 있었다.**
   → **A.** Beds24 에서 `201·301…901` 로 개명(원천이 고쳐짐) / **B.** 우리 정규화에 `N` → `N01`
3. **가부키초 302호 설정 흠.** `452062` 만 `includeBookings` 가 비어 있다(다른 22개는 다 있음).
   둘이 동시에 팔 수 있는 날이 187일 중 0일이라 지금 노출은 없지만, 생기면 더블부킹이 된다.
4. **노출된 시크릿 정리.** 2026-09-25 에 `.env` 전체가 스크린샷으로 찍혔다.
   Slack Webhook URL 과 `BEDS24_WEBHOOK_SECRET` 은 전체가 읽힌다 —
   후자는 프로덕션의 `rates-sync`·`reconcile`·`price-jobs/worker` 를 호출하고 **가짜 예약
   웹훅을 밀어넣을 수 있다.** 바꾸려면 Vercel · GitHub Actions · Beds24 URL **세 곳을 동시에**.
