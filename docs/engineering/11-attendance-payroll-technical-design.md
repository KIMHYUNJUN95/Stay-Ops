...
Status: Refined technical draft (policy baseline confirmed 2026-06-17) · **Step 1 (schema + permission
foundation) implemented 2026-06-17** — migration `202606170001_attendance_payroll.sql`.

## As-built — Step 1 (schema + permission foundation, 2026-06-17)

Built the database + permission foundation so the already-designed attendance UI
(`/mobile/attendance` Home + Capture + Correction, `docs/product/24-attendance-workflow.md`) can be
wired in later steps without a schema rewrite. **No action wiring yet** (no clock-in/out, break,
correction, finalization, dashboard, export, or notification logic) — schema + RLS only.

- **All 11 tables created** exactly per the "Recommended Tables" spec below: `attendance_sites`,
  `attendance_qr_tokens`, `attendance_sessions`, `attendance_breaks`, `attendance_attempt_logs`,
  `attendance_correction_requests`, `attendance_session_audits`, `employment_type_history`,
  `hourly_rate_history`, `attendance_month_snapshots`, `attendance_export_logs`. Numeric coords/rates
  use `numeric`; device/meta fields use `jsonb`.
- **Permission foundation**: `memberships.attendance_payroll_admin boolean not null default false`
  added; `can_manage_attendance_payroll(org)` SECURITY DEFINER helper = platform admin OR active
  `owner`/flagged member. Site master + QR issuance remain **owner-only** (app-enforced).
- **Hard guarantees enforced now (DB)**: one `open` session per user (partial unique on
  `user_id where status='open'`); one active QR token per site (partial unique on
  `site_id where is_active`); correction `image_urls` ≤ 5; status/method/reason/review_state/audit-action
  fields CHECK-constrained to the allowed values; site `allowed_radius_meters` default **100** and
  `wifi_ssids text[]` modeled; lat/long range checks; effective_to ≥ effective_from on history rows.
- **RLS**: read-only policies only (own rows for workers; org-wide for `can_manage_attendance_payroll`;
  QR tokens / attempt logs / session audits / export logs are admin-only; sites readable by any active
  member). No write policies — all writes go through service-role server actions later. See
  `docs/engineering/05-rls-permissions.md` → "Attendance / Payroll tables".
- **Indexes** for the expected UI-backed queries: own history (`organization_id, user_id,
  operating_date desc`), admin review queue (`organization_id, review_state, operating_date desc`),
  status/date filters, correction queue, audit/attempt timelines, per-user month snapshots, export log.
- **TS + shared module**: `src/types/database.ts` updated (11 tables + the membership flag);
  `src/lib/attendance.ts` exports row-type aliases, every status/method/reason union (mirroring the DB
  CHECKs), and schema constants (default radius 100, correction max images 5, gross rounding 10 yen,
  18:30 reminder). No queries/actions/UI in that module yet.
- **Supersedes** the earlier `attendance_events` / `employment_profiles` draft that had been sketched in
  `docs/engineering/04-data-model.md` (now updated to the session-first model).

Remaining for Step 2+: clock-in/out + break + correction server actions (service-role, GPS/QR
validation, Tokyo operating-date, one-open-session + open-break-blocks-clock-out enforcement); own
history + admin review queries; then payroll (employment/rate history management, expected pay,
monthly finalization snapshots, dashboard), export generation, and notifications. Wi-Fi stays inactive.

## As-built — Step 2 (site/QR backend helpers + dev temp-QR, 2026-06-17)

**Scope decision (2026-06-17, user-confirmed):** the owner-only **site master + QR management UI will
live in the WEB DASHBOARD (admin web)**, built later — the app is being finished first. So Step 2 does
**not** ship an admin UI. Instead it delivers (a) the reusable site/QR **backend helpers** the future
web dashboard will call, and (b) a **dev-only temporary-QR tool** so the app's attendance flow can be
tested before that dashboard exists.
...
- **Atomic QR issuance** — migration `202606170002_issue_attendance_qr_fn.sql` adds the plpgsql
  ...
  `replaced_by_token_id` — all in one transaction, so the "one active token per site" partial-unique
...
- **Backend helpers** — `src/lib/attendance-sites.ts` (server-only, service-role; caller-agnostic):
  ...
  (via the RPC) / `revokeAttendanceQr`; `generateAttendanceQrToken()` makes a site-specific
  `att_<base64url>` token. Every helper is **organization-scoped**. They DO NOT check the caller —
  **owner-only enforcement is the caller's responsibility** and is deferred to the web-dashboard server
  actions (`role === 'owner'`, server-side). Wi-Fi SSIDs are stored on the site; Wi-Fi attendance stays
  inactive.
- **Dev temp-QR tool** — `GET /api/dev/attendance/temp-qr` (gated exactly like `/api/dev/seed-login`:
  ...
  logged-in session's org, ensures a temp site (`임시 테스트 현장`, central-Tokyo coords, 100 m radius,
  sample SSID — overridable via `?lat/?lng/?radius/?name/?ssid` on first creation), ensures an active
  QR (or `?reissue=1` to force a fresh one), and renders a **scannable QR** (SVG via the `qrcode`
  dependency, added this step) plus the token; `?format=json` returns the raw data. Not owner-gated —
  it's a local testing tool.

Deferred to the web-dashboard phase: the owner-only site list / create / edit / GPS·radius / Wi-Fi
SSID / QR issue·reissue·revoke UI and its owner-enforcing server actions (which call the helpers
above). Still pending for the worker app (Step 3+): clock-in/out + break + correction execution.
...
- **Home** (`attendance-home.tsx` + `page.tsx`) now renders from REAL data via
  `getCurrentOpenSession` (`src/lib/attendance-sessions.ts`): an open session → 근무 중 with a live
  per-second elapsed timer, real site name + clock-in time, and **퇴근하기 → `/mobile/attendance/capture?mode=out`**;
...
- **Clock-out blocking** — the Step-3 clock-out path now checks for an open break on the session right
  ...
  "휴게 종료 후 퇴근할 수 있어요 → 홈으로" message. (The home also disables 퇴근하기 while on break, so this
  is defense-in-depth.)
- **Home wiring** (`attendance-home.tsx` + `attendance-sessions.ts`): `getCurrentOpenSession` now also
  ...
  `breakCount`. The page derives state: open session **with** an open break → 휴게 중; open session →
  근무 중; none → 출근 전. The 근무 중 state's 휴게 시작 button calls `startBreak`; the 휴게 중 state
  renders live (current break mm:ss, worked = elapsed − total break, running 휴게 합계 + 휴게 횟수) and
  its 휴게 종료 button calls `endBreak`; 퇴근하기 stays disabled while on break. Both buttons
  `router.refresh()` on success. No redesign — same classes/structure; `?state=` previews retained.
- **States rendered for real**: no open session (출근 전), open + not on break (근무 중), open + on break
  (휴게 중). A completed session's break history surfaces later with the history view (Step 5+); breaks
...
Own attendance history is now a real, **self-only** screen. The v2 design handoff had no 이력 frame, so
...
- **History screen** `/mobile/attendance/history` (`src/app/mobile/attendance/history/page.tsx` +
  ...
  `attendance.css` (scoped under `.att`, token-based). A discreet **이력** link was added to the home
  topline. No existing screen was restructured; `?state=` previews and Steps 3–4 wiring are unchanged.
...
form + `…/correction/status` screens (these already existed as prototypes). **Request creation only —
no admin approve/reject, no session mutation, no auto-apply** (Step 7).
...
- **Create action** `createAttendanceCorrectionRequest(input)` (`src/app/mobile/attendance/actions.ts`,
  ...
  Tokyo instants; a single desired site applies to both in/out columns (design's "출/퇴근 동일"); photos
  capped at 5. The request **never touches the session** — it only suggests values.
- **Exception path:** a request with no `sessionId` (reached from the capture failure sheets' "정정 요청"
...
- **Form** (`attendance-correction-form.tsx`) is now controlled: reason chips (Korean ↔ enum), native
  ...
  shared drag-dismiss sheet, memo, and a real photo picker (≤5, compress + preview + remove). Submit
...
- **Status** (`attendance-correction-status.tsx`) is data-driven from a `CorrectionRequestView`
  ...
  steps + recap (대상/사유/희망 시각/장소/첨부/메모) + review block. All four states (requested /
...
- **Self-history surfacing:** `getAttendanceHistory` now attaches the latest `correctionStatus` per
  session (via `getCorrectionStatusBySession`); the history cards/detail show a 정정 요청됨/검토중/승인/반려
  chip, and the detail sheet offers "이 세션 정정 요청" (or "정정 요청 상태 보기").
...
- **Actions** `src/app/admin/attendance/actions.ts` (`"use server"`, service-role, all privilege-gated):
  ...
  - `approveCorrectionRequest({ requestId, finalClockInAt?, finalClockOutAt?, finalSiteId?, comment? })`
    ...
    comment or "정정 요청 승인", before/after JSON) is written. The request is marked `approved`
...
- **Reflection (no UI change):** manual/invalidated sessions already render in the existing flows — the
  ...
  history (수동 chip via `manualCreated`, 무효 chip via `invalid` status). Invalidated records remain
...
data. **No admin PC/web dashboard** (explicit scope rule). The 급여 screen is **new UI in the existing
`.att` language** (the handoff had no 급여 frame; user asked for an arbitrary screen to refine later).
...
- **Calculation layer** `src/lib/attendance-pay.ts` (server-only; pure helpers reusable by the later
  ...
  - `paidSecondsForSession` (worked − closed breaks, ≥0), `roundToNearest10` (monthly final layer only).
...
- **Self pay screen** `/mobile/attendance/pay` (`attendance-pay.tsx`): month nav (`?ym=`), expected-gross
  ...
  states. A 급여 link sits next to 이력 in the home topline. Self-only (server-scoped).
...
- **Rate-data note (Step 9 still pending):** the proper owner/`attendance_payroll_admin` employment-type
  ...
  caller's employment type + rate. Pay shows ¥0 / empty until a rate exists.
...
- **Reopen** `reopenAttendanceMonth({ userId, ym, reason })` (privilege-gated, **reason required**):
  flips the current `finalized` row to `reopened` (kept as history; no `finalized` row remains →
  **expected pay resumes**). Prior finalized history is never destroyed; a later finalize supersedes the
...
- **Self-view reflection:** `getMonthlyPayView` now also reads the current `finalized` snapshot and
  returns a `finalization` block; the pay screen shows the **locked finalized gross + a 확정 badge +
...
- `getPayrollTotals(org, ym)` (`src/lib/attendance-payroll-totals.ts`, server-only) returns the
  ...
  - **`siteTotals`** — expected labor aggregated by **clock-in site** (the documented first-slice rule),
    from each included session's exact gross; null-site bucket = "미지정".
...
- **Export lib** `src/lib/attendance-export.ts` — `runPayrollExport(service, org, actorId, { scope, ym,
  userId? })`:
  - **Privilege enforced here** (`isAttendancePayrollAdmin`; owner / `attendance_payroll_admin`) →
    `forbidden` otherwise. Regular users can never export.
  - **Finalized-only, strict:** gathers `attendance_month_snapshots` with `status='finalized'` for the
    ...
    reopened / superseded / non-finalized are never included. Empty bulk → `empty`; missing per-person →
    `not_finalized`.
  - **Format foundation:** the operator's final Excel template is still pending, so it emits a clean,
    ...
    snapshot fields (대상월 · 직원 · 직원ID · 유급(분) · 유급시간 · 총급여 · 시급구간 · 확정자 · 확정시각).
...
midnight sweep, the employment/rate **management** backend (Step 9), and the owner/admin **web-dashboard
UI** (site/QR · review queue · manual mgmt · rate mgmt · finalize/reopen · totals dashboard · export —
all deferred until the app is complete). The final operator Excel **export template** also remains
...
- **Worker 18:30 reminder:** a **once-per-Tokyo-day home prompt** (shared drag-dismiss sheet) shows when
  the user has an open session and Tokyo time ≥ 18:30 and they haven't answered today. **"근무 중이에요"**
  records `still_working` (suppresses the prompt the rest of the day); **"이미 퇴근했어요"** records
